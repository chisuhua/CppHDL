// src/simulator_init.cpp
// Phase 6: Simulator::initialize() moved out of simulator.cpp.
// Owns: Simulator::initialize() — context setup, data buffer allocation,
//       instruction creation + classification (input/reset/clock/seq/comb),
//       memory port wiring.
#include "simulator.h"
...
void Simulator::initialize() {
    ...
    for (auto *node : eval_list_) {
        ...
        if (node->type() == ch::core::lnodetype::type_lit) {
            ...
            // Directly assign the literal value to avoid issues with assign_truncate
            // when the target has 0 bitwidth (default-constructed sdata_type)
            data_map_[node_id] = lit_value;
            CHDBG("Set literal value(%d) for node %u",
