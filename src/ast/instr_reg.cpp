...
instr_reg::instr_reg(sdata_type *current_buf, uint32_t size,
                     sdata_type *next_buf, sdata_type *clk_edge,
                     sdata_type *clk_en_buf, sdata_type *rst_buf,
                     sdata_type *rst_val_buf)
    : instr_base(size), current_buf_(current_buf), next_buf_(next_buf),
      clk_edge_(clk_edge), clk_en_buf_(clk_en_buf), rst_buf_(rst_buf),
      rst_val_buf_(rst_val_buf), current_node_id_(static_cast<uint32_t>(-1)),
      next_node_id_(static_cast<uint32_t>(-1)) {
    // Store the node IDs instead of the pointers
    CHDBG_FUNC();
    CHINFO("Created register instruction for size=%u", size);
}

// --- instr_reg::eval with data_map lookup and truncation using
// bv_assign_truncate ---
void instr_reg::eval() {
    CHDBG_FUNC();
    ...
    // 检查时钟边沿（直接使用传入的clk_edge值）
    if (clk_edge_->is_zero()) {
        // 没有有效的时钟边沿，不更新寄存器
        return;
    }
