...
namespace ch
{
    ...
    instr_mem::instr_mem(uint32_t node_id, uint32_t addr_width, uint32_t data_width,
        ...
        : instr_base(data_width * depth), node_id_(node_id), addr_width_(addr_width), data_width_(data_width), depth_(depth), is_rom_(is_rom)
    {
    ...
    void instr_mem::write_memory(uint32_t addr, const sdata_type &data, const sdata_type &enable)
    {
        ...
        if (enable.bitwidth() > 1)
        {
            ...
            {
                if (enable.get_bit(byte_idx))
                { // 使用sdata_type的get_bit
    ...
    instr_mem_async_read_port::instr_mem_async_read_port(uint32_t port_id, uint32_t parent_mem_id,
                                                         uint32_t data_width)
        : instr_base(data_width), port_id_(port_id), parent_mem_id_(parent_mem_id), mem_ptr_(nullptr), addr_node_id_(0), enable_node_id_(0), addr_data_ptr_(nullptr), output_data_ptr_(nullptr), enable_data_ptr_(nullptr)
    {

        CHDBG_FUNC();
        CHINFO("Created async memory read port instruction for port_id=%u, parent=%u",
               port_id_, parent_mem_id_);
    }

    void instr_mem_async_read_port::init_port(uint32_t addr_node_id, uint32_t enable_node_id, uint32_t output_node_id, data_map_t &data_map)
    {
    ...
    instr_mem_sync_read_port::instr_mem_sync_read_port(uint32_t port_id, uint32_t parent_mem_id,
                                                       uint32_t data_width)
        : instr_base(data_width), port_id_(port_id), parent_mem_id_(parent_mem_id), last_clk_(false), mem_ptr_(nullptr), clk_node_id_(0), addr_node_id_(0), enable_node_id_(0), clk_data_ptr_(nullptr), addr_data_ptr_(nullptr), output_data_ptr_(nullptr), enable_data_ptr_(nullptr)
    {

        CHDBG_FUNC();
        CHINFO("Created sync memory read port instruction for port_id=%u, parent=%u",
               port_id_, parent_mem_id_);
    }

    void instr_mem_sync_read_port::init_port(uint32_t clk_node_id, uint32_t addr_node_id, uint32_t enable_node_id, uint32_t output_node_id, data_map_t &data_map)
    {
    ...
    instr_mem_write_port::instr_mem_write_port(uint32_t port_id, uint32_t parent_mem_id,
                                               uint32_t data_width)
        : instr_base(data_width), port_id_(port_id), parent_mem_id_(parent_mem_id), last_clk_(false), mem_ptr_(nullptr), clk_node_id_(0), addr_node_id_(0), wdata_node_id_(0), enable_node_id_(0), clk_data_ptr_(nullptr), addr_data_ptr_(nullptr), wdata_ptr_(nullptr), enable_data_ptr_(nullptr)
    {
    ...
    void instr_mem_write_port::init_port(uint32_t clk_node_id, uint32_t addr_node_id,
