...
SC_MODULE(SimpleInitiator) {
    ...
    SC_CTOR(SimpleInitiator) : socket("socket") { SC_THREAD(run); }
    ...
    void run() {
        ...
        std::cout << "Bundle width: " << test_bundle.width() << " bits"
                  << std::endl;
        std::cout << "Bundle payload: "
                  << static_cast<uint64_t>(test_bundle.payload) << std::endl;
