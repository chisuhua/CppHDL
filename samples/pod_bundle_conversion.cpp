...
struct LargeData {
    ...
    bool operator==(const LargeData& other) const {
        return address == other.address && data == other.data && 
               extra == other.extra && flag1 == other.flag1 && flag2 == other.flag2;
    }
};
...
void assign_pod_to_bundle_advanced(const PodType& pod, BundleType& bundle, Simulator& sim) {
    static_assert(std::is_standard_layout_v<PodType> && std::is_trivial_v<PodType>, 
...
PodType assign_bundle_to_pod_advanced(const BundleType& bundle, Simulator& sim) {
    static_assert(std::is_standard_layout_v<PodType> && std::is_trivial_v<PodType>, 
                  "PodType must be a POD struct");
    static_assert(std::is_base_of_v<bundle_base<BundleType>, BundleType>, 
                  "BundleType must be derived from bundle_base");
...
int main() {
    ...
    std::cout << "SimpleData size: " << sizeof(SimpleData) << " bytes (" << (sizeof(SimpleData) * 8) << " bits)" << std::endl;
    std::cout << "LargeData size: " << sizeof(LargeData) << " bytes (" << (sizeof(LargeData) * 8) << " bits)" << std::endl;
    
    // 测试2: 大尺寸POD序列化/反序列化
    std::cout << "\n=== Test 2: Large POD Serialization/Deserialization ===" << std::endl;
    ...
    LargeData deserialized_large = deserialize_pod_from_uint64_array<LargeData>(uint64_array);
    ...
    // 测试3: Bundle创建和分析
    std::cout << "\n=== Test 3: Large Bundle Creation and Analysis ===" << std::endl;
    auto ctx = std::make_unique<ch::core::context>("demo_ctx");
    ch::core::ctx_swap ctx_guard(ctx.get());
    
    large_data_bundle large_bundle;
    std::cout << "LargeDataBundle width: " << large_bundle.width() << " bits" << std::endl;
    
    // 测试4: 通过仿真器设置大尺寸Bundle值
    std::cout << "\n=== Test 4: Large Bundle Value Setting via Simulator ===" << std::endl;
    ...
    public:
        ...
        TestLargeDataModule(ch::Component* parent = nullptr, const std::string& name = "test_large_module")
    ...
    // 使用大尺寸POD数据设置Bundle
    LargeData test_large_data{0x123456789ABCDEF0ULL, 0xABCD1234, 0xEF56, true, false};
    ...
    std::cout << "POD serialized to " << large_uint64_array.size() << " uint64_t values" << std::endl;
    
    // 通过仿真器设置Bundle值（使用第一个值）
    uint64_t first_value = large_uint64_array.empty() ? 0 : large_uint64_array[0];
    std::cout << "Using first uint64_t value: 0x" << std::hex << first_value << std::dec << std::endl;
    
    large_sim.set_bundle_value(test_large_device.instance().io, first_value);
    large_sim.tick();
    
    // 获取Bundle值
    uint64_t large_bundle_value = large_sim.get_bundle_value(test_large_device.instance().io);
    std::cout << "Bundle value from simulator: 0x" << std::hex << large_bundle_value << std::dec << std::endl;
    
    // 将Bundle值转换回POD（注意：这里会丢失部分信息，因为我们只使用了一个uint64_t）
    std::vector<uint64_t> result_array(1, large_bundle_value);
    LargeData result_large_data = deserialize_pod_from_uint64_array<LargeData>(result_array);
    std::cout << "Result POD data (partial): ";
    result_large_data.print();
    
    std::cout << "\nNote: Full conversion requires handling multiple uint64_t values." << std::endl;
    std::cout << "This demonstrates how to work with POD types larger than 64 bits." << std::endl;
