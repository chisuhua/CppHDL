...
int main() {
    ...
    std::cout << "concat(a,b) width: "
              << ch_width_v<decltype(concat_result)> << " bits" << std::endl;
    std::cout << "Expected concat width: "
              << (ch_width_v<decltype(a)> + ch_width_v<decltype(b)>) << " bits"
              << std::endl;

    // Test bit slice operations
    ch_uint<12> data(0b101101011100, "data");
    auto slice = bits<7, 4>(data);
    std::cout << "bits<7,4>(data) width: "
              << ch_width_v<decltype(slice)> << " bits" << std::endl;
    std::cout << "Expected slice width: " << (7 - 4 + 1) << " bits"
              << std::endl;
    ...
    auto sum = x + y;
    std::cout << "x + y width: " << ch_width_v<decltype(sum)> << " bits"
              << std::endl;
    std::cout << "Expected sum width: " << (std::max(7u, 5u) + 1) << " bits"
              << std::endl;
    ...
    std::cout << "concat(p, concat(q, r)) width: "
              << ch_width_v<decltype(nested)> << " bits" << std::endl;
    std::cout << "Expected nested width: " << (2 + 3 + 4) << " bits"
              << std::endl;
