...
int main() {
    ...
    try {
        ...
        std::cout << "   Stream<uint8> width: " << stream8.width() << " bits"
                  << std::endl;
        std::cout << "   Stream<uint16> width: " << stream16.width() << " bits"
                  << std::endl;
        std::cout << "   Stream<uint32> width: " << stream32.width() << " bits"
                  << std::endl;
        ...
        std::cout << "   Nested bundle width: "
                  << get_bundle_width<nested_bundle>() << " bits" << std::endl;
        ...
        std::cout << "   Main serialize() result width: "
                  << serialized_main.width << " bits" << std::endl;

        // 反序列化演示
        auto deserialized_main =
            ch::core::deserialize<custom_data_bundle>(serialized_main);

        std::cout << "   Deserialized bundle created successfully" << std::endl;

        std::cout << "\n🎉 All Bundle Serialization features work correctly!"
                  << std::endl;
        std::cout << "📝 Note: Actual serialization/deserialization requires "
                     "IR node access"
                  << std::endl;
        std::cout << "         and is implemented in the full version."
                  << std::endl;

        std::cout << "\n🔧 Unified Interface Summary:" << std::endl;
        std::cout << "   - Unified interface: serialize()/deserialize()"
                  << std::endl;
        std::cout << "   - All code now uses the same primary interface"
                  << std::endl;
