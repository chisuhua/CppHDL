...
// 定义一个简单的Bundle处理模块
template <typename T> class SimpleBundleModule : public ch::Component {
public:
    fifo_bundle<T> io;

    SimpleBundleModule(ch::Component *parent = nullptr,
                       const std::string &name = "bundle_module")
...
public:
    ...
    // 为传统仿真添加端口
    __io(ch_out<ch_uint<4>> data_out; ch_out<ch_bool> full_out;
         ch_out<ch_bool> empty_out; ch_in<ch_uint<4>> data_in;
         ch_in<ch_bool> push_in; ch_in<ch_bool> pop_in;)
...
int main() {
    std::cout << "Bundle Simulator Demo - demonstrating direct Bundle support "
                 "in simulator"
              << std::endl;
    ...
    std::cout << "  data : " << data_value << " (" << std::bitset<4>(data_value)
              << ")" << std::endl;
    std::cout << "  valid: " << valid_value << " ("
              << std::bitset<1>(valid_value) << ")" << std::endl;
    std::cout << "  ready: " << ready_value << " ("
              << std::bitset<1>(ready_value) << ")" << std::endl;
    ...
    std::cout << "  Result: " << std::bitset<6>(serialized) << " = 0x"
    ...
    std::cout << "  Extracted data : " << extracted_data << " ("
              << std::bitset<4>(extracted_data) << ")" << std::endl;
    std::cout << "  Extracted valid: " << extracted_valid << " ("
              << std::bitset<1>(extracted_valid) << ")" << std::endl;
    std::cout << "  Extracted ready: " << extracted_ready << " ("
              << std::bitset<1>(extracted_ready) << ")" << std::endl;
    ...
    std::cout << "Setting bundle input with POD value: 0x" << std::hex
              << bundle_input << " (" << std::bitset<8>(bundle_input) << ")"
              << std::dec << std::endl;
    ...
    // 直接使用Bundle设置值的新方法
    std::cout << "\nUsing new direct Bundle support in simulator..."
              << std::endl;
    ...
    std::cout << "  data_out : " << static_cast<uint64_t>(data_out)
              << std::endl;
    std::cout << "  full_out : " << static_cast<uint64_t>(full_out)
              << std::endl;
    std::cout << "  empty_out: " << static_cast<uint64_t>(empty_out)
              << std::endl;

    // 获取输出 - 直接通过Bundle
    uint64_t bundle_result =
        sim.get_bundle_value(top_device.instance().bundle_io);
    ...
    std::cout << "  data : "
              << ch::core::ch_width_v<ch::core::ch_uint<4>> << " bits"
              << std::endl;
    std::cout << "  push : "
              << ch::core::ch_width_v<ch::core::ch_bool> << " bits"
              << std::endl;
    std::cout << "  full : "
              << ch::core::ch_width_v<ch::core::ch_bool> << " bits"
              << std::endl;
    ...
    std::cout << "  empty: "
              ...
              << std::endl;
    std::cout << "  Total: "
              << ch::core::get_bundle_width<fifo_bundle<ch_uint<4>>>()
              << " bits" << std::endl;

    std::cout << "\ninterrupt_bundle field widths:" << std::endl;
    std::cout << "  irq: " << ch::core::ch_width_v<ch::core::ch_bool> << " bits"
              << std::endl;
    std::cout << "  ack: " << ch::core::ch_width_v<ch::core::ch_bool> << " bits"
              << std::endl;
    std::cout << "  Total: " << ch::core::get_bundle_width<interrupt_bundle>()
              << " bits" << std::endl;

    // config_bundle示例
    std::cout << "\nconfig_bundle<8, 32> field widths:" << std::endl;
    std::cout << "  addr : "
              << ch::core::ch_width_v<ch::core::ch_uint<8>> << " bits"
              << std::endl;
    std::cout << "  wdata: "
              << ch::core::ch_width_v<ch::core::ch_uint<32>> << " bits"
              << std::endl;
    std::cout << "  rdata: "
              << ch::core::ch_width_v<ch::core::ch_uint<32>> << " bits"
              << std::endl;
    ...
    std::cout << "  ready: "
              ...
              << std::endl;
    std::cout << "  Total: "
              << ch::core::get_bundle_width<config_bundle<8, 32>>() << " bits"
              << std::endl;
    ...
    std::cout << "Created fifo_bundle<ch_uint<4>> with width: "
              << demo_bundle.width() << " bits" << std::endl;

    // 获取Bundle字段信息
    auto fields = demo_bundle.__bundle_fields();
    std::cout << "Bundle has "
              << std::tuple_size_v<decltype(fields)> << " fields" << std::endl;

    std::cout << "\n=== 测试6: 复杂Bundle操作演示 ===" << std::endl;

    // 创建一个config_bundle并演示其操作
    config_bundle<8, 32> config_demo_bundle;
    std::cout << "Created config_bundle<8, 32> with width: "
              << config_demo_bundle.width() << " bits" << std::endl;
    ...
    public:
        ...
        ConfigDevice(ch::Component *parent = nullptr,
                     const std::string &name = "config_device")
            : ch::Component(parent, name) {
            cfg.as_slave();
        }
    ...
    // 使用Bundle设置配置值
    uint64_t config_value = 0x2000000025; // [ready=0, read=1, write=0,
                                          // rdata=0x20, wdata=0x25, addr=0]
    std::cout << "Setting config bundle with value: 0x" << std::hex
              << config_value << std::dec << std::endl;

    config_sim.set_bundle_value(config_device.instance().cfg, config_value);
    config_sim.tick();

    uint64_t config_result =
        config_sim.get_bundle_value(config_device.instance().cfg);
    std::cout << "Config bundle result: 0x" << std::hex << config_result
              << std::dec << std::endl;

    std::cout << "\nDemo completed successfully!" << std::endl;
    std::cout << "This demonstrates how Bundle fields are serialized into bit "
                 "segments"
              << std::endl;
    std::cout << "and how the simulator can work with Bundle interfaces."
              << std::endl;
