...
// 简单的组合逻辑模块示例
template <typename T>
class SimpleLogic : public ch::Component {
public:
    // 使用Bundle作为IO接口
    fifo_bundle<T> io;

    SimpleLogic(ch::Component* parent = nullptr, const std::string& name = "simple_logic")
...
int main() {
    std::cout << "Bundle Top Example - demonstrating direct Bundle usage as top-level IO" << std::endl;
    ...
    std::cout << "This demonstrates how to use Bundles directly as top-level IO interfaces." << std::endl;
    std::cout << "In a full implementation, we would also need to connect these to the simulator." << std::endl;
