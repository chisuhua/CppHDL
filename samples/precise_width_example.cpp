...
#include "component.h"
#include "core/literal.h" // 添加这一行以支持字面量操作符如1_d
#include "module.h"
#include "simulator.h"
#include <iostream>
...
// 示例模块：展示精确宽度支持
template <unsigned N> class PreciseWidthExample : public ch::Component {
public:
    __io(ch_in<ch_uint<N>> in_data;
         ch_out<ch_uint<N + 1>> out_data; // 输出宽度比输入大1
         )

        PreciseWidthExample(ch::Component *parent = nullptr,
                            const std::string &name = "precise_width_example")
...
int main() {
    ...
    try {
        ...
        std::cout << "   Sum result width: "
                  << ch_width_v<decltype(sum)> << " bits" << std::endl;
        std::cout << "   Expected width: 4 bits" << std::endl;
        std::cout << "   Test "
                  << (ch_width_v<decltype(sum)> == 4 ? "PASSED" : "FAILED")
                  << std::endl;
        ...
        std::cout << "   Sum result width: "
                  << ch_width_v<decltype(sum2)> << " bits" << std::endl;
        std::cout << "   Expected width: 8 bits" << std::endl;
        std::cout << "   Test "
                  << (ch_width_v<decltype(sum2)> == 8 ? "PASSED" : "FAILED")
                  << std::endl;
        ...
        std::cout << "   Concat result width: "
                  << ch_width_v<decltype(concatenated)> << " bits" << std::endl;
        std::cout << "   Expected width: 8 bits" << std::endl;
        std::cout << "   Test "
                  << (ch_width_v<decltype(concatenated)> == 8 ? "PASSED"
                                                              : "FAILED")
                  << std::endl;
        ...
        std::cout << "   Expected width: 4 bits" << std::endl;
        std::cout << "   Test "
                  << (ch_width_v<decltype(slice)> == 4 ? "PASSED" : "FAILED")
                  << std::endl;
        ...
        // 设置输入值
        simulator.set_port_value(device.instance().io().in_data,
        ...
        // 获取输出值
        auto output_value =
            simulator.get_port_value(device.instance().io().out_data);
        std::cout << "   Input value: " << 0b1010 << " (decimal)" << std::endl;
        std::cout << "   Output value: " << static_cast<uint64_t>(output_value)
                  << " (decimal)" << std::endl;
        std::cout << "   Expected output: " << (0b1010 + 1) << " (decimal)"
                  << std::endl;
        std::cout << "   Test "
                  << (static_cast<uint64_t>(output_value) == (0b1010 + 1)
                          ? "PASSED"
                          : "FAILED")
                  << std::endl;
