...
class SimpleFifo : public ch::Component {
public:
    static_assert((N & (N - 1)) == 0, "FIFO size must be power of 2");
    
    fifo_bundle<T> io;

    SimpleFifo(ch::Component* parent = nullptr, const std::string& name = "fifo")
...
int main() {
    ...
    for (int cycle = 0; cycle < 3; ++cycle) {
        ...
        std::cout << "Cycle " << cycle << ": bundle_value=0x" << std::hex << bundle_value << std::dec << std::endl;
    }
    
    std::cout << "Minimal FIFO Bundle test completed successfully!" << std::endl;
    return 0;
}