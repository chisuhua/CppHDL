# verilator-backend-e2e Spec (Delta)

> Note: 本文件位于 `openspec/changes/adr-035-phase-3-complete/specs/`，使用 OpenSpec delta 格式（`## ADDED Requirements`）。archive 后会合并到 `openspec/specs/verilator-backend-e2e/spec.md`。
...
Proposed in `openspec/changes/adr-035-phase-3-complete/proposal.md` on 2026-09-21.

Migrated from ADR-035 §7.3 + §7.4 + §7.5 acceptance criteria, with explicit `SHALL` requirements extracted from Phase table stubs in `src/core/verilator_backend.cpp`.

## Validation

| Check | Command | Expected |
|-------|---------|----------|
| Phase 3.3 port binding | `tests/test_verilator_backend.cpp verilator_port_binding_rw` | PASS |
| Phase 3.4 clock model | `tests/test_verilator_backend.cpp verilator_clock_3_eval_model` | PASS |
| Phase 3.5 cache hit | `tests/test_verilator_backend.cpp verilator_sha1_cache_hit_skips_compile` | PASS, < 100ms |
| Phase 3.6 VCD | `tests/test_verilator_backend.cpp verilator_vcd_dump_writes_file` | PASS, file > 1KB |
| 132 ctest 全绿 | `cd CppHDL/build && ctest --output-on-failure` | 132/132 PASS |
| Ported 28/28 | `cd CppHDL && ./run_all_ported_tests.sh` | 28/28 PASS |
| counter e2e | `tests/test_verilator_backend.cpp verilator_e2e_counter_simulator` | counter_value == 50 |
| riscv-mini e2e | (M6 阶段) ChipForge `tests/cpu/test_cpu_verilator_sim.cpp` | DMem[0x1000] == 1 |
