 ...
 */
template<unsigned N>
class OneHotDecoder : public ch::Component {
public:
    // 确保至少有一个位
    static_assert(N > 0, "OneHotDecoder must have at least 1 bit");

    // 输出宽度应该足够容纳N个可能的值 (0 到 N-1)
    static constexpr unsigned OUTPUT_WIDTH = (N > 1) ? ch::core::compute_bit_width(N - 1) : 1;

    // 定义IO端口
    __io(
        ch_in<ch::core::ch_uint<N>> in;           // N位 one-hot 输入
        ch_out<ch::core::ch_uint<OUTPUT_WIDTH>> out;  // 解码后的输出值
    )
     ...
     * @param name 组件名称
     */
    OneHotDecoder(ch::Component* parent = nullptr, const std::string& name = "onehot_decoder") 
