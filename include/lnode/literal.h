...
struct lit_bin {
    ...
    static constexpr unsigned chr2int(char c) { return static_cast<unsigned>(c - '0'); }
...
struct lit_hex {
    ...
    static constexpr bool is_escape(char c) { return (c == '\'' || c == 'x' || c == 'X'); }
...
template <typename Lit, uint64_t Value, char... Chars>
struct lit_value;

template <typename Lit, uint64_t Value>
struct lit_value<Lit, Value> {
...
struct lit_value<Lit, Value, Char, Chars...> {
    static_assert(Lit::is_digit(Char) || Lit::is_escape(Char), "invalid character in literal");
    static constexpr uint64_t new_value =
        Lit::is_escape(Char) ? Value : (Value * Lit::base() + Lit::chr2int(Char));
    static constexpr uint64_t value = lit_value<Lit, new_value, Chars...>::value;
};
...
template <typename Lit, unsigned Acc, char... Chars>
struct lit_size;

template <typename Lit, unsigned Acc>
struct lit_size<Lit, Acc> {
...
struct lit_size<Lit, Acc, Char, Chars...> {
    static_assert(Lit::is_digit(Char) || Lit::is_escape(Char), "invalid character in literal");
    static constexpr unsigned add = Lit::is_escape(Char) ? 0u : Lit::digit_width();
...
template <char... Chars>
inline constexpr unsigned lit_dec_size_v = compute_bit_width(lit_dec_value_v<Chars...>);
