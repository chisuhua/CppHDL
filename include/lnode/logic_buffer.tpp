...
template <typename T> logic_buffer<T>::logic_buffer() : node_impl_(nullptr) {}

template <typename T>
logic_buffer<T>::logic_buffer(const logic_buffer &other)
...
// 位操作增强实现
template <typename T> auto logic_buffer<T>::operator[](unsigned index) const {
    return bit_select(index, *static_cast<const T *>(this));
}
...
template <typename T> auto logic_buffer<T>::msb() const {
    return bit_select(T::width - 1, *static_cast<const T *>(this));
}

template <typename T> auto logic_buffer<T>::lsb() const {
    return bit_select(0, *static_cast<const T *>(this));
}
...
template <typename T> bool logic_buffer<T>::is_ones() const {
    return (*static_cast<const T *>(this)) == ~T(0);
}

template <typename T> bool logic_buffer<T>::is_power_of_two() const {
    auto val = *static_cast<const T *>(this);
    return (val != T(0)) && ((val & (val - T(1))) == T(0));
}

// 获取位宽实现
template <typename T> constexpr unsigned logic_buffer<T>::width() {
    return T::width;
}
