...
#include "core/reg.h"       // 确保 to_operand 可见
...
void next_assignment_proxy<T>::operator=(const U &value) const {
    ...
    CHDBG("[next_assignment_proxy::operator=] Assigning value (node ID: %d) to "
          "regimpl node ID %d",
          (src_impl ? static_cast<int>(src_impl->id()) : -1),
          (regimpl_node_ ? static_cast<int>(regimpl_node_->id()) : -1));

    if (regimpl_node_ && src_impl &&
        regimpl_node_->type() == lnodetype::type_reg) {
        ...
        CHERROR("[next_assignment_proxy::operator=] Error: regimpl_node_ is "
                "null, not a regimpl, or src_impl is null!");
    }
}

// --- ch_reg implementation ---

// Template constructor for general types (handles ch_literal, ch_uint, ch_reg,
// etc.)
template <typename T>
template <typename U>
ch_reg<T>::ch_reg(const U &initial_value, const std::string &name,
...
template <typename T>
ch_reg<T>::ch_reg(const std::string &name, const std::source_location &sloc) {
...
template <typename T> ch_reg<T>::operator lnode<T>() const {
    return lnode<T>(this->impl());
}

template <typename T> lnode<T> ch_reg<T>::as_ln() const {
