...
// 常量节点构建实现
template <typename T>
lnodeimpl *node_builder::build_literal(T value, const std::string &name,
    ...
    if constexpr (std::is_arithmetic_v<T>) {
        ...
        return ctx->create_literal(
            sdata, prefixed_name_helper(name, instance().name_prefix_), sloc);
    } else {
        // 处理 sdata_type
        if (instance().statistics_enabled_) {
            ++instance().statistics_->literals_built;
        ...
        return ctx->create_literal(
            value, prefixed_name_helper(name, instance().name_prefix_), sloc);
    }
}
...
lnodeimpl *node_builder::build_input(const std::string &name,
    ...
    return ctx->create_input(
        size, prefixed_name_helper(name, instance().name_prefix_), sloc);
}
...
// 寄存器节点构建实现
template <typename T>
std::pair<regimpl *, proxyimpl *>
node_builder::build_register(lnodeimpl *init_val, lnodeimpl *next_val,
    ...
    proxyimpl *proxy_node = ctx->create_node<proxyimpl>(
        reg_node, prefixed_name_helper("_" + name, instance().name_prefix_),
        sloc);
...
lnodeimpl *node_builder::build_mux(const lnode<Cond> &cond,
...
template <typename T>
lnodeimpl *node_builder::build_bit_select(const lnode<T> &operand,
                                          uint32_t index,
                                          const std::string &name,
                                          const std::source_location &sloc) {
    ...
    proxyimpl *proxy_node = ctx->create_node<proxyimpl>(
        op_node, prefixed_name_helper("_" + name, instance().name_prefix_),
        sloc);
...
template <typename T>
lnodeimpl *node_builder::build_bits(const lnode<T> &operand, uint32_t msb,
                                    uint32_t lsb, const std::string &name,
                                    const std::source_location &sloc) {
    ...
    // Encode range as single 64-bit value (msb in upper 32 bits, lsb in lower
    // 32 bits)
    const uint64_t range_encoded =
        (static_cast<uint64_t>(msb) << 32) | static_cast<uint64_t>(lsb);

    // Create operation node specifically for bits extraction
    opimpl *op_node = ctx->create_node<opimpl>(
        result_width, ch_op::bits_extract, false, operand.impl(),
        ctx->create_literal(sdata_type(range_encoded, 64), name + "_range",
                            sloc),
        prefixed_name_helper(name, instance().name_prefix_), sloc);
...
lnodeimpl *node_builder::build_bit_extract(const lnode<T> &operand,
    ...
    auto *ctx = ctx_curr_;
    if (!ctx) {
        CHERROR("[node_builder] No active context for bit extract operation "
                "creation");
        return nullptr;
    }

    if (!operand.impl() || !index.impl()) {
        CHERROR("[node_builder] Invalid operand or index for bit extract "
                "operation");
        return nullptr;
    }
    ...
    opimpl *op_node = ctx->create_node<opimpl>(
        result_width, ch_op::bits_extract, false, operand.impl(), index.impl(),
        prefixed_name_helper(name, instance().name_prefix_), sloc);
...
lnodeimpl *node_builder::build_operation(ch_op op, const lnode<T> &operand,
    ...
    CHDBG_FUNC();
    if (instance().debug_mode_) {
        CHINFO("[node_builder] Building unary operation %d",
               static_cast<int>(op));
    }
