...
void bv_init(T* dst, uint32_t size) {
  ...
  std::fill_n(dst, num_words, T{0}); // 👈 改为零初始化
...
template <bool is_signed, typename T>
int bv_cmp_scalar(const T* lhs, uint32_t lhs_offset,
                  const T* rhs, uint32_t rhs_offset,
                  uint32_t length) {
...
template <bool is_signed, typename T>
int bv_cmp_vector(const T* lhs, uint32_t lhs_offset,
                  const T* rhs, uint32_t rhs_offset,
                  uint32_t length) {
    ...
    for (int32_t i = static_cast<int32_t>(num_words) - 1; i >= 0; --i) {
        ...
        T lhs_word = extract_bits(lhs, lhs_offset + i * WORD_BITS, bits_in_this_word);
        T rhs_word = extract_bits(rhs, rhs_offset + i * WORD_BITS, bits_in_this_word);

        if (lhs_word < rhs_word) return -1;
        if (lhs_word > rhs_word) return 1;
    }
    return 0;
}
...
int bv_cmp(const T* lhs, uint32_t lhs_offset,
    ...
    if (length <= bitwidth_v<T>) {
        return bv_cmp_scalar<is_signed, T>(lhs, lhs_offset, rhs, rhs_offset, length);
    } else {
        return bv_cmp_vector<is_signed, T>(lhs, lhs_offset, rhs, rhs_offset, length);
    }
}
...
*/
template <bool is_signed, typename T>
void bv_shl_scalar(T* out, uint32_t out_size, const T* in, uint32_t in_size, uint32_t dist) {
    T value;
    if constexpr (is_signed) {
...
template <bool is_signed, typename T>
void bv_shl_vector(T* out, uint32_t out_size, const T* in, uint32_t in_size, uint32_t dist) {
    static constexpr uint32_t WORD_SIZE = bitwidth_v<T>;
    auto in_num_words = ceildiv<int32_t>(in_size, WORD_SIZE);
    auto out_num_words = ceildiv<int32_t>(out_size, WORD_SIZE);
    ...
    if (shift_bits == 0) {
        for (int32_t i = out_num_words - 1; i >= shift_words; --i) {
             out[i] = (i - shift_words < in_num_words) ? in[i - shift_words] : T(0);
        }
    } else {
        for (int32_t i = out_num_words - 1; i > shift_words; --i) {
            out[i] = (i - shift_words < in_num_words) ? in[i - shift_words] : T(0);
        ...
        if (shift_words < out_num_words) {
            out[shift_words] = (0 < in_num_words) ? (in[0] << shift_bits) : T(0);
        }
    }
    bv_clear_extra_bits(out, out_size);
}
...
template <bool is_signed, typename T>
void bv_shl(T* out, uint32_t out_size, const T* in, uint32_t in_size, uint32_t dist) {
    static constexpr uint32_t WORD_SIZE = bitwidth_v<T>;
