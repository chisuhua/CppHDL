...
template <typename T> void bv_copy_scalar(T *out, const T *in) {
    out[0] = in[0];
}

template <typename T> void bv_copy_vector(T *out, const T *in, uint32_t size) {
    static constexpr uint32_t WORD_SIZE = bitwidth_v<T>;
...
template <typename T> void bv_copy(T *out, const T *in, uint32_t size) {
    static constexpr uint32_t WORD_SIZE = bitwidth_v<T>;
...
// --- Unified entry point ---
template <bool is_signed, typename T>
int bv_cmp(const T *lhs, uint32_t lhs_offset, const T *rhs, uint32_t rhs_offset,
           uint32_t length) {
    if (length == 0)
        return 0;
