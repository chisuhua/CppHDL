...
template <typename T>
constexpr T low_bit_mask(uint32_t bits) {
    if (bits == 0) return T{0};
    if (bits >= bitwidth_v<T>) return T{~T{0}}; // all ones
    return (T{1} << bits) - T{1};
}
...
inline void ch_check(bool condition, const char* msg = "Assertion failed") {
    if (!condition) {
        // In a real library, this might throw an exception or call a user-defined handler
        // For now, use standard assert for simplicity
