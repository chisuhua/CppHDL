...
public:
  ...
  bitvector& operator*=(const bitvector& other) {
    ...
    // 结果可能需要调整大小以适应乘法结果，但这里假设 resize 到 max 已足够或由 bv_mul 内部处理
    // 如果 bv_mul 可能产生更长的结果，可能需要重新计算 size_ 并再次 resize
  ...
  bitvector& operator|=(const bitvector& other) {
    ...
    // 对于 OR 操作，如果 other 更大，可能需要 sign_extend 或 zero_extend 本对象
    // bv_or 通常会处理对齐，但确保 extra bits 被正确清除
    bv_clear_extra_bits(words_, size_);
    return *this;
  }
...
// 加法
template <typename word_t>
bitvector<word_t> operator+(bitvector<word_t> lhs, const bitvector<word_t>& rhs) {
    lhs += rhs;
    return lhs;
}
...
// Assigns bits from source (src) to destination (dst), truncating or zero-extending
// to fit exactly dst_size bits.
// This function ensures that only the lower dst_size bits from src (or src itself if src is smaller)
// are copied to dst, and dst's size is considered fixed at dst_size.
//
// Template Parameters:
...
/*
template <typename T>
void bv_assign_truncate(T* dst, uint32_t dst_size, const T* src, uint32_t src_size) {
    static constexpr uint32_t WORD_SIZE = bitwidth_v<T>; // Bits per word (e.g., 32 or 64)
    ...
    if (src_size == 0) {
        ...
        bv_clear_extra_bits(dst, dst_size); // Ensure any partial high word is cleared
        return;
    }

    // Determine the actual number of bits to copy: the minimum of source and destination size.
    ...
    if (copy_rem_bits > 0) {
        ...
        T mask = (T(1) << copy_rem_bits) - 1; // Create mask for the remaining bits (e.g., 0b1111 for 4 bits)
    ...
    // This also ensures any unused high bits within the last word of dst are zeroed.
    if (dst_size > copy_bits) {
        // Zero out words in dst that are completely beyond the copied data
        uint32_t zero_start_word = ceildiv(copy_bits, WORD_SIZE); // First word index to start zeroing
        uint32_t zero_end_word   = ceildiv(dst_size, WORD_SIZE); // One past the last word index of dst
        if (zero_end_word > zero_start_word) {
             std::fill(dst + zero_start_word, dst + zero_end_word, T(0));
        }
        // Note: If copy_bits landed exactly on a word boundary (copy_rem_bits == 0),
        // zero_start_word might equal the index of the word containing the last copied bit.
        // In that case, we still need to clear extra bits within that word if dst_size
        // doesn't fill it completely.
    }

    // --- Final cleanup: Clear any extra bits beyond dst_size within the last word ---
...
void bv_assign_truncate(T* dst, uint32_t dst_size, const T* src, uint32_t src_size) {
    ...
    // --- DEBUG PRINTS ---
    // std::cout << "[DEBUG bv_assign_truncate] Called with dst=" << static_cast<void*>(dst) 
    //           << ", dst_size=" << dst_size 
    ...
    if (dst_size == 0) {
        // Nothing to assign to.
        // --- DEBUG PRINT ---
        // std::cout << "[DEBUG bv_assign_truncate] dst_size is 0, returning." << std::endl;
        // --- END DEBUG PRINT ---
        return;
    }
    ...
    if (src_size == 0) {
        ...
        uint32_t num_dst_words = ceildiv(dst_size, WORD_SIZE);
        // --- DEBUG PRINT ---
        // std::cout << "[DEBUG bv_assign_truncate] src_size is 0, clearing " << num_dst_words << " words." << std::endl;
        // --- END DEBUG PRINT ---
        std::fill_n(dst, num_dst_words, T(0));
        bv_clear_extra_bits(dst, dst_size); // Ensure any partial high word is cleared
        return;
    ...
    uint32_t copy_bits = std::min(src_size, dst_size);
    // --- DEBUG PRINT ---
    // std::cout << "[DEBUG bv_assign_truncate] copy_bits (min of src/dst size) = " << copy_bits << std::endl;
    // --- END DEBUG PRINT ---
    ...
    uint32_t copy_rem_bits   = copy_bits % WORD_SIZE;
    // --- DEBUG PRINT ---
    // std::cout << "[DEBUG bv_assign_truncate] copy_full_words=" << copy_full_words << ", copy_rem_bits=" << copy_rem_bits << std::endl;
    // --- END DEBUG PRINT ---

    // --- Copy full words ---
    if (copy_full_words > 0) {
        // --- DEBUG PRINT ---
        // std::cout << "[DEBUG bv_assign_truncate] Copying " << copy_full_words << " full words." << std::endl;
        // --- END DEBUG PRINT ---
        std::memcpy(dst, src, copy_full_words * sizeof(T));
    }
    ...
    if (copy_rem_bits > 0) {
        ...
        // std::cout << "[DEBUG bv_assign_truncate] Partial word copy: index=" << partial_word_index 
        //           << ", mask=0x" << std::hex << mask << std::dec
        //           << ", src_val_before_mask=0x" << std::hex << src_val_before_mask << std::dec
        //           << ", masked_val=0x" << std::hex << masked_val << std::dec << std::endl;
        // --- END DEBUG PRINT ---
        dst[partial_word_index] = (src[partial_word_index] & mask);
        // --- DEBUG PRINT ---
        // std::cout << "[DEBUG bv_assign_truncate] Assigned masked value 0x" << std::hex << (src[partial_word_index] & mask) << std::dec 
        //           << " to dst[" << partial_word_index << "]" << std::endl;
        // T dst_val_after_assign = dst[partial_word_index];
        // std::cout << "[DEBUG bv_assign_truncate] dst[" << partial_word_index << "] after assign = 0x" << std::hex << dst_val_after_assign << std::dec << std::endl;
        // --- END DEBUG PRINT ---
    }
    ...
    if (dst_size > copy_bits) {
        ...
        uint32_t zero_end_word   = ceildiv(dst_size, WORD_SIZE); // One past the last word index of dst
        // --- DEBUG PRINT ---
        // std::cout << "[DEBUG bv_assign_truncate] dst_size > copy_bits, zeroing words from " << zero_start_word << " to " << zero_end_word << std::endl;
        // --- END DEBUG PRINT ---
        if (zero_end_word > zero_start_word) {
             std::fill(dst + zero_start_word, dst + zero_end_word, T(0));
        }
    ...
    // This is crucial to ensure dst conforms strictly to dst_size bits.
    // --- DEBUG PRINT ---
    // std::cout << "[DEBUG bv_assign_truncate] Calling bv_clear_extra_bits(dst, " << dst_size << ")" << std::endl;
    // --- END DEBUG PRINT ---
    bv_clear_extra_bits(dst, dst_size);
    // --- DEBUG PRINT ---
    // std::cout << "[DEBUG bv_assign_truncate] Finished. Final dst[0] = 0x" << std::hex << dst[0] << std::dec << std::endl;
    // --- END DEBUG PRINT ---
}


template<typename T>
void bv_add_truncate(bitvector<T>* dst, const bitvector<T>* lhs, const bitvector<T>* rhs) {
    // Implementation using bv_add_vector/bv_add_scalar and ensuring result fits dst->size()
    // ...
    // Example sketch:
    ...
    bv_add<false>(temp_result.words(), result_width, lhs->words(), lhs->size(), rhs->words(), rhs->size());
    // Truncate the result to fit dst
    bv_assign_truncate(dst->words(), dst->size(), temp_result.words(), result_width);
    // Or, if dst is pre-sized correctly, use bv_assign_truncate directly on dst
    // bv_assign_truncate(dst->words(), dst->size(), temp_result.words(), temp_result.size());
}
...
void bv_assign_truncate(T* dst, uint32_t dst_size, const T* src, uint32_t src_size) {
    ...
    if (src_size == 0 || src == nullptr) {
        // Source is empty or invalid. Clear the destination buffer relevant to dst_size.
        ...
        // Clear extra bits in the last word if dst_size is not a multiple of WORD_SIZE
        bv_clear_extra_bits(dst, dst_size);
        return;
    }
    ...
    if (copy_rem_bits > 0) {
        ...
        // Be careful with shift if copy_rem_bits == WORD_SIZE, which would create an invalid shift.
        // However, if copy_rem_bits == WORD_SIZE, copy_full_words would be incremented,
        // and copy_rem_bits would be 0, so this branch wouldn't execute.
        ...
        // No need to explicitly mask higher bits of dst beyond copy_rem_bits
        // within this word, as bv_clear_extra_bits below will handle the very last bits of dst.
    }

    // --- Clear bits in dst beyond the copied range (if dst is larger than src/copy_bits) ---
    // This also ensures any unused high bits within the last word of dst are zeroed
    if (dst_size > copy_bits) {
        // Zero out words in dst that are completely beyond the copied data
        uint32_t zero_start_word = ceildiv(copy_bits, WORD_SIZE); // First word index to start zeroing
    ...
    // --- Final cleanup: Clear any extra bits beyond dst_size within the last word ---
    // This is crucial to ensure dst conforms strictly to dst_size bits.
    // It clears any bits in the highest word of the destination that are beyond the specified dst_size.
    // This handles the case where dst_size is not a multiple of WORD_SIZE
    // and also ensures cleanliness if dst_size <= copy_bits.
    bv_clear_extra_bits(dst, dst_size);
}
// --- END Helper function ---
...
// Copies bits from src to dst, ensuring only the lower dst->size() bits are copied.
// Handles cases where src is wider or narrower than dst.
template<typename T>
...
template<typename T>
void bv_add_truncate(bitvector<T>* dst, const bitvector<T>* lhs, const bitvector<T>* rhs) {
    if (!dst || !lhs || !rhs) return; // Safety check
    ...
    if (dst_size == 0) return; // Nothing to compute
    ...
    // If the destination is wide enough for the full result, perform the operation directly
    if (dst_size >= full_result_width) {
        bv_add<false>(dst->words(), dst_size, lhs->words(), lhs_size, rhs->words(), rhs_size);
        bv_clear_extra_bits(dst->words(), dst_size); // Ensure extra bits are cleared
        return;
    }
    ...
    bv_add<false>(temp_words.data(), full_result_width,
                  lhs->words(), lhs_size,
                  rhs->words(), rhs_size);

    // Truncate the full result from the temporary buffer into the destination
    bv_assign_truncate<T>(dst->words(), dst_size,
                          temp_words.data(), full_result_width);
...
// Truncates or zero-extends the result to fit exactly dst->size() bits.
template<typename T>
void bv_sub_truncate(bitvector<T>* dst, const bitvector<T>* lhs, const bitvector<T>* rhs) {
    if (!dst || !lhs || !rhs) return; // Safety check
    ...
    // Calculate the width needed for the full subtraction result before truncation
    // max operand width (subtraction doesn't typically increase width beyond operands)
    uint32_t full_result_width = std::max(lhs_size, rhs_size);

    // If the destination is wide enough for the full result, perform the operation directly
    if (dst_size >= full_result_width) {
        bv_sub<false>(dst->words(), dst_size, lhs->words(), lhs_size, rhs->words(), rhs_size);
        bv_clear_extra_bits(dst->words(), dst_size); // Ensure extra bits are cleared
        return;
    }

    // If the destination is narrower, compute the full result first, then truncate.
    uint32_t temp_num_words = ceildiv(full_result_width, WORD_SIZE);
    std::vector<T> temp_words(temp_num_words, T(0));

    bv_sub<false>(temp_words.data(), full_result_width,
                  lhs->words(), lhs_size,
                  rhs->words(), rhs_size);
...
// Truncates or zero-extends the result to fit exactly dst->size() bits.
template<typename T>
void bv_mul_truncate(bitvector<T>* dst, const bitvector<T>* lhs, const bitvector<T>* rhs) {
    if (!dst || !lhs || !rhs) return; // Safety check
    ...
    // Calculate the width needed for the full multiplication result before truncation
    // sum of operand widths
    uint32_t full_result_width = lhs_size + rhs_size;

    // If the destination is wide enough for the full result, perform the operation directly
    if (dst_size >= full_result_width) {
        bv_mul<false>(dst->words(), dst_size, lhs->words(), lhs_size, rhs->words(), rhs_size);
        bv_clear_extra_bits(dst->words(), dst_size); // Ensure extra bits are cleared
        return;
    }
    ...
    bv_mul<false>(temp_words.data(), full_result_width,
                  lhs->words(), lhs_size,
                  rhs->words(), rhs_size);
...
// Truncates or zero-extends the result to fit exactly dst->size() bits.
template<typename T>
void bv_div_truncate(bitvector<T>* dst, const bitvector<T>* lhs, const bitvector<T>* rhs) {
    if (!dst || !lhs || !rhs) return; // Safety check
    ...
    // If the destination is wide enough for the full result, perform the operation directly
    if (dst_size >= full_result_width) {
        bv_div<false>(dst->words(), dst_size, lhs->words(), lhs_size, rhs->words(), rhs_size);
        bv_clear_extra_bits(dst->words(), dst_size); // Ensure extra bits are cleared
        return;
    }
    ...
    bv_div<false>(temp_words.data(), full_result_width,
                  lhs->words(), lhs_size,
                  rhs->words(), rhs_size);
...
// Truncates or zero-extends the result to fit exactly dst->size() bits.
template<typename T>
void bv_mod_truncate(bitvector<T>* dst, const bitvector<T>* lhs, const bitvector<T>* rhs) {
    if (!dst || !lhs || !rhs) return; // Safety check
    ...
    // If the destination is wide enough for the full result, perform the operation directly
    if (dst_size >= full_result_width) {
        bv_mod<false>(dst->words(), dst_size, lhs->words(), lhs_size, rhs->words(), rhs_size);
        bv_clear_extra_bits(dst->words(), dst_size); // Ensure extra bits are cleared
        return;
    }
    ...
    bv_mod<false>(temp_words.data(), full_result_width,
                  lhs->words(), lhs_size,
                  rhs->words(), rhs_size);
...
// Truncates or zero-extends the result to fit exactly dst->size() bits.
template<typename T>
void bv_and_truncate(bitvector<T>* dst, const bitvector<T>* lhs, const bitvector<T>* rhs) {
    if (!dst || !lhs || !rhs) return; // Safety check
    ...
    // If the destination is wide enough for the full result, perform the operation directly
    if (dst_size >= full_result_width) {
        bv_and<false>(dst->words(), dst_size, lhs->words(), lhs_size, rhs->words(), rhs_size);
        bv_clear_extra_bits(dst->words(), dst_size); // Ensure extra bits are cleared
        return;
    }
    ...
    bv_and<false>(temp_words.data(), full_result_width,
                  lhs->words(), lhs_size,
                  rhs->words(), rhs_size);
...
template<typename T>
void bv_or_truncate(bitvector<T>* dst, const bitvector<T>* lhs, const bitvector<T>* rhs) {
    if (!dst || !lhs || !rhs) return; // Safety check
    ...
    if (dst_size >= full_result_width) {
        bv_or<false>(dst->words(), dst_size, lhs->words(), lhs_size, rhs->words(), rhs_size);
    ...
    bv_or<false>(temp_words.data(), full_result_width,
                 lhs->words(), lhs_size,
                 rhs->words(), rhs_size);
...
template<typename T>
void bv_xor_truncate(bitvector<T>* dst, const bitvector<T>* lhs, const bitvector<T>* rhs) {
    if (!dst || !lhs || !rhs) return; // Safety check
    ...
    if (dst_size >= full_result_width) {
        bv_xor<false>(dst->words(), dst_size, lhs->words(), lhs_size, rhs->words(), rhs_size);
        bv_clear_extra_bits(dst->words(), dst_size);
        return;
    }
    ...
    bv_xor<false>(temp_words.data(), full_result_width,
                  lhs->words(), lhs_size,
                  rhs->words(), rhs_size);
...
template <typename T>
void bv_inv_truncate(T* dst, uint32_t dst_size, const T* src, uint32_t src_size) {
    // ... (checks) ...
    // Use bv_inv which should handle truncation/extension correctly
    bv_inv<false, T>(dst, dst_size, src, src_size); // Assuming bv_inv truncates result to out_size
...
// Truncates or zero-extends the result to fit exactly dst->size() bits.
template<typename T>
bool bv_not_truncate(bitvector<T>* dst) {
    if (!dst ) return false; // Safety check

    static constexpr uint32_t WORD_SIZE = bitwidth_v<T>;
    const uint32_t dst_size = dst->size();

    if (dst_size == 0) return false; // Nothing to compute
...
// Truncates or zero-extends the result to fit exactly dst->size() bits.
template<typename T>
void bv_shl_truncate(bitvector<T>* dst, const bitvector<T>* src, uint32_t dist) {
    if (!dst || !src) return; // Safety check
    ...
    // Left shift result width is src width + dist (but capped by implementation limits)
    // For truncation, we just perform the shift and then truncate.
...
// Truncates or zero-extends the result to fit exactly dst->size() bits.
template<typename T>
void bv_shr_truncate(bitvector<T>* dst, const bitvector<T>* src, uint32_t dist) {
    if (!dst || !src) return; // Safety check
    ...
    // Right shift result width is src width (logical right shift zeros high bits)
...
// --- 相等 (==) ---
template <typename word_t>
bool bv_eq_truncate(const bitvector<word_t>& lhs, const bitvector<word_t>& rhs) {
...
// --- 不等 (!=) ---
template <typename word_t>
bool bv_ne_truncate(const bitvector<word_t>& lhs, const bitvector<word_t>& rhs) {
    // a != b 等价于 !(a == b)
...
// --- 小于 (<) ---
template <typename word_t>
bool bv_lt_truncate(const bitvector<word_t>& lhs, const bitvector<word_t>& rhs) {
...
// --- 小于等于 (<=) ---
template <typename word_t>
bool bv_le_truncate(const bitvector<word_t>& lhs, const bitvector<word_t>& rhs) {
    // a <= b 等价于 !(b < a)
    // This relies on bv_lt's correct handling of size differences.
    return !bv_gt_truncate(lhs, rhs); // Delegate to bv_gt (a <= b iff !(b < a) is equivalent to !(a > b))
    // Or equivalently: return !bv_lt(rhs, lhs);
}

// --- 大于 (>) ---
template <typename word_t>
bool bv_gt_truncate(const bitvector<word_t>& lhs, const bitvector<word_t>& rhs) {
    // a > b 等价于 b < a
    // This relies on bv_lt's correct handling of size differences.
...
// --- 大于等于 (>=) ---
template <typename word_t>
bool bv_ge_truncate(const bitvector<word_t>& lhs, const bitvector<word_t>& rhs) {
    // a >= b 等价于 !(a < b)
    // This relies on bv_lt's correct handling of size differences.
...
inline std::string to_bitstring(const ch::internal::bitvector<T>& bv) {
    std::string s;
    for (size_t i = 0; i < bv.size(); ++i) s += bv[i] ? '1' : '0';
    // Reverse to show LSB on the right, MSB on the left (conventional representation)
    // Optional: std::reverse(s.begin(), s.end()); 
    // Or read backwards for LSB-first indexing:
