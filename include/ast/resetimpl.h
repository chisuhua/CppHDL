...
public:
    resetimpl(uint32_t id, const sdata_type &init_value, reset_type rtype,
              ...
              context *ctx)
        : lnodeimpl(id, lnodetype::type_reset, 1, ctx, name,
                    sloc) // 复位通常是1位输入
          ,
          init_value_(init_value), reset_type_(rtype),
          is_async_(rtype == reset_type::async_high ||
                    rtype == reset_type::async_low) {}
    ...
    bool is_active_high() const {
        return reset_type_ == reset_type::sync_high ||
               reset_type_ == reset_type::async_high;
    }
    bool is_active_low() const {
        return reset_type_ == reset_type::sync_low ||
               reset_type_ == reset_type::async_low;
    }
    ...
    std::string to_string() const override {
        ...
        oss << name_ << " (reset, ";
        switch (reset_type_) {
        case reset_type::sync_high:
            oss << "sync_high";
            break;
        case reset_type::sync_low:
            oss << "sync_low";
            break;
        case reset_type::async_high:
            oss << "async_high";
            break;
        case reset_type::async_low:
            oss << "async_low";
            break;
        }
        oss << ")";
