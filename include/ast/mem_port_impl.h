...
public:
    mem_port_impl(uint32_t id, memimpl* parent, uint32_t port_id, mem_port_type type,
        ...
        : lnodeimpl(id, 
                   (type == mem_port_type::write) ? lnodetype::type_mem_write_port : lnodetype::type_mem_read_port,
                   size, ctx, name, sloc)
        , parent_mem_(parent)
        , port_id_(port_id)
        , port_type_(type)
    ...
    lnodeimpl* enable() const { return has_enable() ? src(enable_idx_) : nullptr; }

    // 克隆支持
    lnodeimpl* clone(context* new_ctx, const clone_map& cloned_nodes) const override = 0;
};
...
public:
    mem_read_port_impl(uint32_t id, memimpl* parent, uint32_t port_id, mem_port_type type,
                       uint32_t size, lnodeimpl* cd, lnodeimpl* addr, 
                       lnodeimpl* enable, lnodeimpl* data_output,
                       const std::string& name, const std::source_location& sloc, context* ctx)
        : mem_port_impl(id, parent, port_id, type, size, cd, addr, enable, name, sloc, ctx)
        , data_output_idx_(-1) {
...
public:
    mem_write_port_impl(uint32_t id, memimpl* parent, uint32_t port_id,
                        ...
                        const std::string& name, const std::source_location& sloc, context* ctx)
        : mem_port_impl(id, parent, port_id, mem_port_type::write, size, cd, addr, enable, name, sloc, ctx)
        , wdata_idx_(-1) {
    ...
    lnodeimpl* wdata() const { return wdata_idx_ != -1 ? src(wdata_idx_) : nullptr; }
