...
template <unsigned N>
CLAResult<N> carry_lookahead_adder(ch_uint<N> a, ch_uint<N> b, ch_bool carry_in = false) {
    ...
    for (unsigned i = 0; i < N; ++i) {
        ...
        carries = carries | (ch_uint<N>(current_carry) << make_literal(i));
...
ch_uint<2*N> wallace_tree_multiplier(ch_uint<N> a, ch_uint<N> b) {
    ...
    if constexpr (N == 1) {
        ...
        for (unsigned i = 0; i < N; ++i) {
            ...
            ch_uint<N> partial_product = select(b_bit, a << make_literal(i), 0_d);
...
ch_uint<2*N> booth_multiplier(ch_uint<N> a, ch_uint<N> b) {
    ...
    for (unsigned i = 0; i < N; i += 2) {
        ...
        ch_bool bit_i_plus_2 = i+2 < N+1 ? bit_select(extended_multiplier, i+2) : false;
        
        // Booth编码
        ch_uint<2> booth_code = select(bit_i, 1_d, 0_d) | (select(bit_i_plus_1, 2_d, 0_d));
        
        // 根据Booth编码执行操作
        ch_uint<2*N> partial_val = 0_d;
        switch (booth_code.value()) {
            case 0: // 00 -> 0
                partial_val = 0_d;
                break;
            case 1: // 01 -> +1
                partial_val = ch_uint<2*N>(multiplicand) << make_literal(i);
                break;
            case 2: // 10 -> -1
                partial_val = (0_d - ch_uint<2*N>(multiplicand)) << make_literal(i);
                break;
            case 3: // 11 -> 0
                partial_val = 0_d;
                break;
...
template <unsigned N>
DividerResult<N> non_restoring_divider(ch_uint<N> dividend, ch_uint<N> divisor) {
...
ch_uint<(N+1)/2> square_root_calculator(ch_uint<N> input) {
    ...
    if (N < 32) {
        ...
        max_sqrt = make_literal(std::min(est, (unsigned)high.value()));
    }
    high = max_sqrt;
