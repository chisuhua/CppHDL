...
dual_port_ram(ch_uint<ADDR_WIDTH> addr_a, ch_uint<DATA_WIDTH> din_a,
    ...
    result.dout_a = select(we_a, din_a, read_port_a);  // 如果写使能有效，返回写入数据，否则返回读出数据
...
sync_fifo(ch_uint<DATA_WIDTH> din, ch_bool wr_en, ch_bool rd_en,
    ...
    when(true) {
        ...
        // 更新计数
        if_(wr_active && !rd_active) { count_reg = count_reg + 1_d; }
...
public:
    ...
    CamResult<ADDR_WIDTH, DATA_WIDTH> search(ch_uint<DATA_WIDTH> search_data) {
        ...
        for (unsigned i = 0; i < (1U << ADDR_WIDTH); ++i) {
            ch_uint<DATA_WIDTH> stored_data = memory.sread(make_literal(i), true_d);
    ...
    void write(ch_uint<ADDR_WIDTH> addr, ch_uint<DATA_WIDTH> data,
        ...
        // 更新有效位
        ch_uint<(1U << ADDR_WIDTH)> mask = ch_uint<(1U << ADDR_WIDTH)>(1_d) << addr;
...
public:
    ...
    TcamResult<ADDR_WIDTH, DATA_WIDTH> search(ch_uint<DATA_WIDTH> search_data) {
        ...
        for (unsigned i = 0; i < (1U << ADDR_WIDTH); ++i) {
            ch_uint<DATA_WIDTH> stored_data = memory.sread(make_literal(i), true_d);
            ch_uint<DATA_WIDTH> mask = mask_memory.sread(make_literal(i), true_d);
            ch_bool is_valid = bit_select(valid_bits, i);
    ...
    void write(ch_uint<ADDR_WIDTH> addr, ch_uint<DATA_WIDTH> data,
        ...
        // 更新有效位
        ch_uint<(1U << ADDR_WIDTH)> valid_mask = ch_uint<(1U << ADDR_WIDTH)>(1_d) << addr;
        ch_uint<(1U << ADDR_WIDTH)> new_valid = 
