...
template <unsigned DATA_WIDTH, unsigned ADDR_WIDTH>
SyncFifoResult<DATA_WIDTH, ADDR_WIDTH> sync_fifo(
    ch_bool clk,
    ch_bool rst,
    ch_bool wren,
    ...
    // 内部存储器
    ch_ram<1 << ADDR_WIDTH, DATA_WIDTH> memory("sync_fifo_memory");
    ...
    // 写入操作
    ch_bool write_enable = wren && !full(count);
    memory.write(clk, write_ptr, din, write_enable);
    
    // 更新写指针
    ch_uint<ADDR_WIDTH> next_write_ptr = select(
        write_enable, 
        write_ptr + 1_d, 
        write_ptr
    );
    ...
    // 读取操作
    ch_bool read_enable = rden && !empty(count);
    ch_uint<DATA_WIDTH> output_data = memory.read(read_ptr);
    
    // 更新读指针
    ch_uint<ADDR_WIDTH> next_read_ptr = select(
        read_enable, 
        read_ptr + 1_d, 
        read_ptr
    );
    ...
    // 更新计数器
    ch_uint<ADDR_WIDTH+1> next_count = count;
    if (write_enable && !read_enable) {
        next_count = count + 1_d;
    ...
    result.empty = empty(count);
    result.full = full(count);
...
template <unsigned DATA_WIDTH, unsigned ADDR_WIDTH>
FwftFifoResult<DATA_WIDTH, ADDR_WIDTH> fwft_fifo(
    ch_bool clk,
    ch_bool rst,
    ch_bool wren,
    ch_uint<DATA_WIDTH> din,
    ...
    // 内部存储器
    ch_ram<1 << ADDR_WIDTH, DATA_WIDTH> memory("fwft_fifo_memory");
    ...
    // 读取操作
    ch_bool read_enable = rden && !empty(count);
    ch_uint<DATA_WIDTH> next_output = memory.read(read_ptr);
    ...
    // 更新输出寄存器
    ch_uint<DATA_WIDTH> new_output = select(
        empty(count),  // 如果为空，则输出保持不变
        output_reg,    // 否则更新为下一个可读取的数据
        next_output
    );
    output_reg->next = select(
        read_enable || empty(count + 1_d),  // 如果读使能或下个周期为空
        new_output,                         // 更新输出
        output_reg                          // 否则保持不变
    );
...
template <unsigned DATA_WIDTH, unsigned ADDR_WIDTH>
AsyncFifoResult<DATA_WIDTH, ADDR_WIDTH> async_fifo(
    ch_bool wr_clk,
    ch_bool wr_rst,
    ch_bool wren,
    ch_uint<DATA_WIDTH> din,
    ...
    // 内部存储器
    ch_ram<1 << ADDR_WIDTH, DATA_WIDTH> memory("async_fifo_memory");
    
    // 写时钟域
    ch_reg<ch_uint<ADDR_WIDTH>> wr_ptr(0_d, "async_fifo_wr_ptr");
    ch_reg<ch_uint<ADDR_WIDTH>> rd_ptr_gray_sync(0_d, "async_fifo_rd_ptr_gray_sync");
    ch_reg<ch_uint<ADDR_WIDTH+1>> wr_count(0_d, "async_fifo_wr_count");
    
    // 读时钟域
    ch_reg<ch_uint<ADDR_WIDTH>> rd_ptr(0_d, "async_fifo_rd_ptr");
    ch_reg<ch_uint<ADDR_WIDTH>> wr_ptr_gray_sync(0_d, "async_fifo_wr_ptr_gray_sync");
    ...
    ch_bool wr_full = (to_gray(rd_ptr_gray_sync) == wr_ptr_gray + (1 << make_literal(ADDR_WIDTH-1)));
...
template <unsigned DATA_WIDTH, unsigned ADDR_WIDTH>
LifoResult<DATA_WIDTH, ADDR_WIDTH> lifo_stack(
    ch_bool clk,
    ch_bool rst,
    ch_bool push,
    ch_uint<DATA_WIDTH> din,
    ...
    // 内部存储器
    ch_ram<1 << ADDR_WIDTH, DATA_WIDTH> memory("lifo_memory");
    ...
    // push操作
    ch_bool push_enable = push && !full(stack_ptr);
    memory.write(clk, stack_ptr, din, push_enable);
    
    // 更新栈指针
    ch_uint<ADDR_WIDTH> next_stack_ptr = stack_ptr;
    if (push_enable && !pop) {
        next_stack_ptr = stack_ptr + 1_d;
    } else if (!push_enable && pop && !empty(stack_ptr)) {
        next_stack_ptr = stack_ptr - 1_d;
    }
    stack_ptr->next = next_stack_ptr;
    
    // pop操作
    ch_bool pop_enable = pop && !empty(stack_ptr);
    ch_uint<ADDR_WIDTH> pop_addr = select(pop_enable, stack_ptr - 1_d, stack_ptr);
    ch_uint<DATA_WIDTH> output_data = memory.read(pop_addr);
    
    LifoResult<DATA_WIDTH, ADDR_WIDTH> result;
    result.empty = empty(stack_ptr);
    result.full = full(stack_ptr);
    result.q = output_data;
