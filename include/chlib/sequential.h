 ...
 */
template <unsigned N>
ch_uint<N> register_(ch_bool en, ch_uint<N> d,
 ...
 */
template <unsigned N>
ch_uint<N> register_(ch_bool rst, ch_bool en, ch_uint<N> d,
                     const std::string &name = "register") {
    ch_reg<ch_uint<N>> reg(0_d, name);
 ...
 */
template <unsigned N>
ch_uint<N> dff_(ch_bool en, ch_uint<N> d, const std::string &name = "dff") {
    ch_reg<ch_uint<N>> reg(0_d, name);
 ...
 */
template <unsigned N>
ch_uint<N> dff_(ch_bool rst, ch_bool en, ch_uint<N> d,
 ...
 */
template <unsigned N>
ch_uint<N> johnson_counter(ch_bool rst, ch_bool en,
                           const std::string &name = "johnson_counter") {
    ch_reg<ch_uint<N>> counter(0_d, name);
...
ch_uint<N> ring_counter(ch_bool en, const std::string &name = "ring_counter") {
    ...
    ch_uint<N> shifted = (counter << 1_d) | msb;

    counter->next = select(en, shifted, counter);

    ch_uint<N> shifted_tap(shifted, "shifted");
    ch_uint<1> msb_tap(msb, "msb");
    ch_uint<N> counter_tap(counter, "counter");
...
edge_detector(ch_bool signal, const std::string &name = "edge_detector") {
    ...
    ch_uint<1> current_signal_tap(current_signal, "current_signal");
    ch_uint<1> prev_signal_tap(prev_signal, "prev_signal");
    ch_uint<1> pos_edge(result.pos_edge, "pos_edge");
    ch_uint<1> neg_edge(result.neg_edge, "neg_edge");
