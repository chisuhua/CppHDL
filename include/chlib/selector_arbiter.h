...
PrioritySelectorResult<N> priority_selector(ch_uint<N> request) {
    ...
    for (unsigned i = 0; i < N; ++i) {
        ...
        ch_uint<N> grant_one_hot = ch_uint<N>(1_d) << make_literal(i);
        
        // 如果当前位有请求且尚未确定优先级更高的请求，则授予当前位
        result.grant = select(req_at_i && !result.valid, grant_one_hot, result.grant);
        result.valid = select(req_at_i, true, result.valid);
    }
 ...
 */
template <unsigned N>
struct RoundRobinArbiterResult {
    ch_uint<N> grant;      // 授予信号，one-hot编码
    ch_bool valid;         // 是否有有效请求
...
template <unsigned N>
RoundRobinArbiterResult<N> round_robin_arbiter(
    ch_uint<N> request, 
    ...
    for (unsigned i = 0; i < N; ++i) {
        ...
        result.grant = select(req_at_pos && !result.valid, grant_one_hot, result.grant);
    ...
    if (result.valid) {
        ...
        for (unsigned i = 0; i < N; ++i) {
            ...
            next_priority = select(req_at_pos && (result.grant == (ch_uint<N>(1_d) << make_literal(pos))), 
 ...
 */
template <unsigned N>
PrioritySelectorResult<N> round_robin_selector(ch_uint<N> request, ch_uint<N> last_grant) {
    ...
    // 从last_grant的下一位开始查找
    unsigned start_pos = (last_grant.value() + 1) % N;
    
    // 先从start_pos到高位查找
    for (unsigned i = 0; i < N; ++i) {
        unsigned pos = (start_pos + i) % N;
        ch_bool req_at_pos = bit_select(request, pos);
        ch_uint<N> grant_one_hot = ch_uint<N>(1_d) << make_literal(pos);
