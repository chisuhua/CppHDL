...
#include "../ch.hpp"
#include "bundle/stream_bundle.h"  // 使用正确的stream_bundle.h路径
#include <array>
#include <type_traits>
...
template<typename T>
struct FragmentBundle : public ch::core::bundle_base<FragmentBundle<T>> {
    ...
    // 从节点和偏移构造（用于嵌套bundle）
    explicit FragmentBundle(ch::core::lnodeimpl* node, unsigned base_offset = 0) 
        : ch::core::bundle_base<FragmentBundle<T>>(node) {
...
struct StreamBundle : public ch::core::bundle_base<StreamBundle<DataT>> {
    ...
    // 从节点和偏移构造（用于嵌套bundle）
    explicit StreamBundle(ch::core::lnodeimpl* node, unsigned base_offset = 0) 
        : ch::core::bundle_base<StreamBundle<DataT>>(node) {
        // 初始化字段，如果是bundle类型则递归处理
        initialize_fields_with_offset(base_offset);
    }
    
    explicit StreamBundle(const std::string& prefix = "stream") {
        this->set_name_prefix(prefix);
    }
    ...
    void do_recursive_connect(const OtherStream& src) {
        ...
        if (this->get_role() == ch::core::bundle_role::master) {
            ...
            if constexpr (ch::core::is_bundle_v<DataT>) {
                ch::core::bundle_field_traits<DataT>::connect(src.payload, this->payload);
            } else {
...
template<typename T>
ch::Flow<T> fragment_to_payload(ch::Flow<FragmentBundle<T>> flow) {
    ch::Flow<T> result;
...
template<typename T>
ch::Flow<FragmentBundle<T>> payload_to_fragment(T payload, ch::core::ch_bool last) {
    ch::Flow<FragmentBundle<T>> result;
...
std::array<ch::Flow<FragmentBundle<T>>, N> fragment_sequence(const std::array<T, N>& data) {
    std::array<ch::Flow<FragmentBundle<T>>, N> result;
...
// 检查Fragment是否为最后一个
template<typename T>
ch::core::ch_bool is_last_fragment(ch::Flow<FragmentBundle<T>> flow) {
    return flow.value().payload.last;
}

// 从Flow<Fragment<T>>中提取last信号
template<typename T>
ch::core::ch_bool get_last_signal(ch::Flow<FragmentBundle<T>> flow) {
    return flow.value().payload.last;
}

// 从Flow<Fragment<T>>中提取fragment数据
template<typename T>
T get_fragment_data(ch::Flow<FragmentBundle<T>> flow) {
