...
std::array<Flow<FragmentBundle<T>>, N> fragment_sequence(const std::array<T, N>& data) {
...
// 检查Fragment是否为最后一个
template<typename T>
ch_bool is_last_fragment(Flow<FragmentBundle<T>> flow) {
    return flow.value().payload.last;
}

// 从Flow<FragmentBundle<T>>中提取last信号
template<typename T>
ch_bool get_last_signal(Flow<FragmentBundle<T>> flow) {
    return flow.value().payload.last;
}

// 从Flow<FragmentBundle<T>>中提取fragment数据
template<typename T>
T get_fragment_data(Flow<FragmentBundle<T>> flow) {
