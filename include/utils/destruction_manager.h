...
public:
        ...
        if (!in_static_destruction()) {
            ...
            for (auto *sim : simulators_) {
                if (sim) {
                    ...
                    std::cout << "Notifying simulator " << sim
                              << " about context destruction" << std::endl;
                }
            }
        }
    }
    ...
    void pre_static_destruction() {
        ...
        for (auto *sim : simulators_) {
            if (sim) {
                ...
                // Mark simulator as disconnected to prevent operations during
                // destruction We can't directly call disconnect because of
    ...
    bool is_in_static_destruction() const {
        return const_cast<destruction_manager *>(this)
            ->in_static_destruction_flag();
    }
