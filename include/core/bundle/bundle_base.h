...
template<typename Derived>
class bundle_base : public bundle {
public:
    ...
    virtual void set_name_prefix(const std::string& prefix) {
        const auto& fields = derived()->__bundle_fields();
        std::apply([&](auto&&... f) {
            ((set_field_name(derived()->*(f.ptr), prefix + "." + std::string(f.name))), ...);
        }, fields);
    }
...
protected:
    ...
    // 方向控制辅助函数
    template<typename T>
    void set_field_direction(T& field, input_direction) {
    ...
    template<typename T>
    void set_field_direction(T& field, output_direction) {
    ...
    // 更新字段验证辅助函数
    template<typename T>
    bool check_field_valid(const T& field) const {
    ...
    template<typename Field1, typename Field2, typename Field3, typename Field4>
    void make_output(Field1& field1, Field2& field2, Field3& field3, Field4& field4) {
        set_field_direction(field1, output_direction{});
        set_field_direction(field2, output_direction{});
        set_field_direction(field3, output_direction{});
    ...
    template<typename Field1, typename Field2, typename Field3, typename Field4>
    void make_input(Field1& field1, Field2& field2, Field3& field3, Field4& field4) {
        set_field_direction(field1, input_direction{});
        set_field_direction(field2, input_direction{});
        set_field_direction(field3, input_direction{});
