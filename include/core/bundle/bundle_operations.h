// include/core/bundle_operations.h
...
struct bundle_slice_view : public bundle_base<bundle_slice_view<Start, Count, BundleT>> {
    ...
    constexpr auto slice_fields(const Tuple& tuple) const {
        if constexpr (C == 1) {
            ...
            return std::make_tuple(std::get<S>(tuple), std::get<S+1>(tuple), std::get<S+2>(tuple));
        } else {
            static_assert(C <= 3, "Currently only supports up to 3 fields in slice");
...
struct bundle_concat : public bundle_base<bundle_concat<Bundle1, Bundle2>> {
    ...
    bundle_concat(const std::string& prefix) {
        this->set_name_prefix(prefix);
    }
    
    constexpr auto __bundle_fields() const {
        return std::tuple_cat(first.__bundle_fields(), second.__bundle_fields());
    }
