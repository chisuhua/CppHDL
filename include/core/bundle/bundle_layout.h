...
// 字段布局信息
template <typename BundleT> struct bundle_layout {
...
private:
    ...
    static constexpr auto calculate_field_layout(FieldTuple fields,
        ...
        return std::make_tuple([&current_offset, fields]() constexpr {
            using FieldType =
                std::remove_cvref_t<decltype(T{}.*(std::get<I>(fields).ptr))>;
            constexpr unsigned field_width = get_field_width<FieldType>();
            auto result = bundle_field_with_layout<T, FieldType>{
                std::get<I>(fields).name, std::get<I>(fields).ptr,
...
// 获取Bundle布局信息的便捷函数
template <typename BundleT> constexpr auto get_bundle_layout() {
...
constexpr unsigned find_field_offset(const LayoutTuple &layout,
    ...
    unsigned offset = 0;
    bool found = ((std::string_view(std::get<I>(layout).name) ==
                           std::string_view(field_name)
                       ? (offset = std::get<I>(layout).offset, true)
                       : false) ||
                  ...);
