...
template <typename T> struct next_assignment_proxy;

template <typename T> struct next_proxy;
...
public:
    ...
    // Constructors
    template <typename U>
    ch_reg(const U &initial_value, const std::string &name = "reg",
...
template <typename T> struct ch_width_impl<ch_reg<T>, void> {
    static constexpr unsigned value = ch_width_v<T>;
};

template <typename T> struct ch_width_impl<const ch_reg<T>, void> {
    static constexpr unsigned value = ch_width_v<T>;
};

template <typename T> struct ch_width_impl<volatile ch_reg<T>, void> {
    static constexpr unsigned value = ch_width_v<T>;
};

template <typename T> struct ch_width_impl<const volatile ch_reg<T>, void> {
    static constexpr unsigned value = ch_width_v<T>;
};
