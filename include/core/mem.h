...
public:
    ...
    static constexpr unsigned addr_width = (N > 1) ? (32 - __builtin_clz(N - 1)) : 1;
...
public:
    // 构造函数 - 无初始值
    explicit ch_mem(const std::string& name = "mem",
    ...
    // 构造函数 - 带初始值（从vector）
    template<typename U>
    ch_mem(const std::vector<U>& init_data, 
           const std::string& name = "mem",
           const std::source_location& sloc = std::source_location::current()) {
        static_assert(std::is_integral_v<U>, "Initial data must be integral type");
    ...
    ch_mem(const std::array<U, M>& init_data,
           const std::string& name = "mem",
           const std::source_location& sloc = std::source_location::current()) {
        static_assert(std::is_integral_v<U>, "Initial data must be integral type");
        std::vector<sdata_type> init_sdata = create_init_data(init_data);
    ...
    static ch_mem<T, N> make_rom(const Container& init_data,
    ...
    read_port aread(const ch_uint<addr_width>& addr,
                   ...
                   const std::source_location& sloc = std::source_location::current()) const {
        auto addr_lnode = get_lnode(addr);
        
        // 创建数据输出节点
        auto* ctx = mem_node_->ctx();
        auto* data_output = ctx->create_output(data_width, name + "_data", sloc);
        
        // 创建异步读端口节点
        auto* port_impl = ctx->create_mem_read_port(
            mem_node_, mem_node_->next_port_id(),
            data_width, nullptr, addr_lnode.impl(), nullptr, data_output,
            name, sloc);
    ...
    read_port sread(const ch_uint<addr_width>& addr,
        ...
        // 创建同步读端口节点
        auto enable_impl = is_litimpl_one(enable_lnode.impl()) ? nullptr : enable_lnode.impl();
        auto* port_impl = ctx->create_mem_read_port(
            mem_node_, mem_node_->next_port_id(),
            data_width, cd, addr_lnode.impl(), enable_impl, data_output,
            name, sloc);
    ...
    write_port write(const ch_uint<addr_width>& addr, 
                    const U& data,
        ...
        auto* port_impl = ctx->create_mem_write_port(
            mem_node_, mem_node_->next_port_id(),
            data_width, cd, addr_lnode.impl(), data_lnode.impl(), enable_impl,
            name, sloc);
...
private:
    void init_memory(const std::string& name,
        ...
        mem_node_ = ctx->create_memory(
            addr_width, data_width, N, 1, true, is_rom,
    ...
    std::vector<sdata_type> create_init_data(const Container& data) {
        ...
        static_assert(std::is_integral_v<value_type>, "Container value type must be integral");
...
namespace ch { namespace core {
    template<typename T, unsigned N>
    struct ch_width_impl<ch_mem<T, N>, void> {
