...
#pragma message "Including core/literal.h"
#include <cstdint>
#include <type_traits>
...
struct ch_literal {
    ...
    constexpr ch_literal(int v) noexcept
        : ch_literal(v, compute_width(v)) {}
    ...
    constexpr ch_literal() noexcept
        : value(0), actual_width(1u) {}
    ...
    constexpr operator ch_uint<N>() const {
        static_assert(N >= actual_width, "Literal width exceeds target ch_uint width");
