...
TEST_CASE("ch_mem: basic construction", "[mem][construction][basic]") {
   ...
   /* 
    SECTION("Construction with array init data") {
        std::array<uint32_t, 3> init_data = {0x11111111, 0x22222222, 0x33333333};
        ch_mem<ch_uint<16>, 64> mem(init_data, "test_mem");
        REQUIRE(mem.impl() != nullptr);
        REQUIRE(mem.impl()->init_data().size() == 3);
    }
    */
...
public:
    ...
    test_context(const std::string& name = "test_ctx") : context(name) {
        ...
        default_reset = create_reset(sdata_type(1, 1), resetimpl::reset_type::async_low, "test_rst");
...
TEST_CASE("memimpl: basic functionality", "[mem][impl][basic]") {
    ...
    SECTION("Memory node creation") {
        ...
        auto* mem_node = ctx.create_memory(8, 32, 256, 1, true, false, init_data, "test_mem");
    ...
    SECTION("Memory node with no init data") {
        ...
        auto* mem_node = ctx.create_memory(10, 16, 1024, 1, false, true, empty_init, "test_rom");
        REQUIRE(mem_node != nullptr);
...
TEST_CASE("mem_port_impl: node creation", "[mem][port][impl]") {
    ...
    SECTION("Read port node creation") {
        ...
        auto* read_port = ctx.create_mem_read_port(
            mem_node, 0, 32, nullptr, addr_node, enable_node, data_output, "read_port");
        
        REQUIRE(read_port != nullptr);
        REQUIRE(read_port->parent() == mem_node);
        REQUIRE(read_port->port_id() == 0);
        REQUIRE(read_port->port_type() == mem_port_type::async_read);
        REQUIRE(read_port->data_output() == data_output);
    }
    ...
    SECTION("Write port node creation") {
        ...
        auto* write_port = ctx.create_mem_write_port(
            mem_node, 1, 32, nullptr, addr_node, data_node, enable_node, "write_port");
...
TEST_CASE("ch_mem: port connections", "[mem][ports][connections]") {
    ...
    SECTION("Multiple write ports") {
        ch_uint<8> addr1(0), addr2(1_d);
        ch_uint<32> data1(0x11111111_h), data2(0x22222222_h);
        ch_reg<ch_bool> enable(true);
...
TEST_CASE("ch_mem: initialization data handling", "[mem][init][data]") {
    ...
    SECTION("Array initialization") {
        std::array<uint32_t, 3> init_values = {0x11111111, 0x22222222, 0x33333333};
...
TEST_CASE("ch_mem: error handling", "[mem][error][handling]") {
    ...
    SECTION("Port creation with different address widths") {
        ...
        ch_uint<16> large_addr(0);
        ch_uint<8> small_addr(0);
