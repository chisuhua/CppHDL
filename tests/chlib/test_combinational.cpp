...
TEST_CASE("Combinational: priority encoder function", "[combinational][priority_encoder]") {
    ...
    SECTION("Single bit set") {
        ...
        REQUIRE(simulator.get_value(result) == 4);
    }
    ...
    SECTION("Multiple bits set - priority to lower") {
        ...
        REQUIRE(simulator.get_value(result) == 1);
    }
...
TEST_CASE("Combinational: binary encoder function", "[combinational][binary_encoder]") {
    auto ctx = std::make_unique<ch::core::context>("test_binary_encoder");
    ch::core::ctx_swap ctx_swapper(ctx.get());

    SECTION("Single bit set") {
        ch_uint<8> input(0b00100000_d);
...
TEST_CASE("Combinational: binary decoder function", "[combinational][binary_decoder]") {
    ...
    SECTION("Decode index 3") {
        ...
        REQUIRE(simulator.get_value(result) == 0b00001000_d);
    }
    ...
    SECTION("Decode index 0") {
        ...
        REQUIRE(simulator.get_value(result) == 0b00000001_d);
    }
}
...
TEST_CASE("Combinational: demux function", "[combinational][demux]") {
    ...
    SECTION("Demux to position 2") {
        ...
        ch_uint<2> sel(2_d);
        std::array<ch_uint<4>, 4> outputs = demux<4, 4>(input, sel);
        
        ch::Simulator sim(ctx.get());
        sim.tick();
        
        REQUIRE(simulator.get_value(outputs[0]) == 0);
        REQUIRE(simulator.get_value(outputs[1]) == 0);
        REQUIRE(simulator.get_value(outputs[2]) == 0b1010_d);
        REQUIRE(simulator.get_value(outputs[3]) == 0);
    }
}
...
TEST_CASE("Combinational: full adder function", "[combinational][full_adder]") {
    ...
    SECTION("0 + 0 + 0") {
        ...
        REQUIRE(simulator.get_value(result.sum) == false);
        REQUIRE(simulator.get_value(result.carry_out) == false);
    }
    ...
    SECTION("1 + 1 + 1") {
        ...
        REQUIRE(simulator.get_value(result.sum) == true);
        REQUIRE(simulator.get_value(result.carry_out) == true);
    }
...
TEST_CASE("Combinational: ripple carry adder function", "[combinational][ripple_adder]") {
    ...
    SECTION("Simple addition without carry") {
        ...
        REQUIRE(simulator.get_value(result.sum) == 8);
        REQUIRE(simulator.get_value(result.carry_out) == false);
    }
    ...
    SECTION("Addition with carry") {
        ...
        REQUIRE(simulator.get_value(result.sum) == 1);
        REQUIRE(simulator.get_value(result.carry_out) == true);
    }
    ...
    SECTION("Addition with initial carry") {
        ...
        REQUIRE(simulator.get_value(result.sum) == 9);
        REQUIRE(simulator.get_value(result.carry_out) == false);
    }
}
...
TEST_CASE("Combinational: comparator function", "[combinational][comparator]") {
    ...
    SECTION("A > B") {
        ...
        REQUIRE(simulator.get_value(result.greater) == true);
        REQUIRE(simulator.get_value(result.equal) == false);
        REQUIRE(simulator.get_value(result.less) == false);
    }
    ...
    SECTION("A < B") {
        ...
        REQUIRE(simulator.get_value(result.greater) == false);
        REQUIRE(simulator.get_value(result.equal) == false);
        REQUIRE(simulator.get_value(result.less) == true);
    }
    ...
    SECTION("A == B") {
        ...
        REQUIRE(simulator.get_value(result.greater) == false);
        REQUIRE(simulator.get_value(result.equal) == true);
        REQUIRE(simulator.get_value(result.less) == false);
    }
}
...
TEST_CASE("Combinational: multiplexer function", "[combinational][mux]") {
    ...
    SECTION("8-to-1 mux selection") {
        std::array<ch_uint<4>, 8> inputs = {
            1_d, 2_d, 3_d, 4_d, 5_d, 6_d, 7_d, 8_d
        };
        ch_uint<3> sel(5_d);
    ...
    SECTION("16-to-1 mux selection") {
        ch_uint<4> result = mux16to1<4>(
            1_d, 2_d, 3_d, 4_d, 5_d, 6_d, 7_d, 8_d,
            9_d, 10_d, 11_d, 12_d, 13_d, 14_d, 15_d, 16_d,
            10_d
        );
        ...
        REQUIRE(simulator.get_value(result) == 11);
    }
}

TEST_CASE("Combinational: equals and range check functions", "[combinational][comparison]") {
...
TEST_CASE("Combinational: leading zero detector", "[combinational][leading_zero]") {
...
TEST_CASE("Combinational: leading one detector", "[combinational][leading_one]") {
...
TEST_CASE("Combinational: edge cases", "[combinational][edge]") {
    ...
    SECTION("Zero values addition") {
        ...
        REQUIRE(simulator.get_value(result.sum) == 0);
        REQUIRE(simulator.get_value(result.carry_out) == false);
    }
}