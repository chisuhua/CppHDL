...
TEST_CASE("Arithmetic: basic add function", "[arithmetic][add]") {
    ...
    SECTION("Simple addition") {
        ...
        REQUIRE(simulator.get_value(result) == 8);
    }
    ...
    SECTION("Addition with carry") {
        ...
        REQUIRE(simulator.get_value(result) == 17 % 16);  // 4-bit wraparound
    }
}
...
TEST_CASE("Arithmetic: add with carry function", "[arithmetic][add_carry]") {
    ...
    SECTION("Add with carry in") {
        ...
        REQUIRE(simulator.get_value(result.sum) == 0);  // 7+8+1=16, wraps to 0 in 4-bit
        REQUIRE(simulator.get_value(result.carry_out) == true);
    }
}

TEST_CASE("Arithmetic: basic subtract function", "[arithmetic][subtract]") {
    ...
    SECTION("Subtraction with borrow") {
        ...
        REQUIRE(simulator.get_value(result) == 14);  // 3-5 = -2, wraps to 14 in 4-bit
    }
}

TEST_CASE("Arithmetic: sub with borrow function", "[arithmetic][sub_borrow]") {
    ...
    SECTION("Subtract with no borrow") {
        ...
        SubWithBorrowResult<4> result = sub_with_borrow<4>(a, b, borrow_in);
        
        ch::Simulator sim(ctx.get());
        sim.tick();
        
        REQUIRE(simulator.get_value(result.diff) == 5);
        REQUIRE(simulator.get_value(result.borrow_out) == false);
    }
    ...
    SECTION("Subtract with borrow") {
        ...
        REQUIRE(simulator.get_value(result.diff) == 13);  // 3-5-1 = -3, wraps to 13 in 4-bit
        REQUIRE(simulator.get_value(result.borrow_out) == true);
    }
}
...
TEST_CASE("Arithmetic: multiply function", "[arithmetic][multiply]") {
    ...
    SECTION("Simple multiplication") {
        ...
        REQUIRE(simulator.get_value(result) == 15);
    }
    ...
    SECTION("Larger multiplication") {
        ...
        REQUIRE(simulator.get_value(result) == 120);
    }
}
...
TEST_CASE("Arithmetic: compare function", "[arithmetic][compare]") {
    ...
    SECTION("Equal comparison") {
        ...
        REQUIRE(simulator.get_value(result.not_equal) == false);
...
TEST_CASE("Arithmetic: abs function", "[arithmetic][abs]") {
    ...
    SECTION("Negative number (2's complement)") {
        ...
        ch_uint<4> a(0b1011_d);  // -5 in 2's complement
        ch_uint<4> result = abs<4>(a);
...
TEST_CASE("Arithmetic: shift functions", "[arithmetic][shift]") {
    ...
    SECTION("Left shift") {
        ch_uint<8> a(0b00000101_d);  // 5
        ch_uint<8> result = left_shift<8>(a, 2);
        
        ch::Simulator sim(ctx.get());
        sim.tick();
        
        REQUIRE(simulator.get_value(result) == 0b00010100_d);  // 20
    }
    
    SECTION("Logical right shift") {
        ch_uint<8> a(0b01010000_d);  // 80
        ch_uint<8> result = logical_right_shift<8>(a, 3);
        
        ch::Simulator sim(ctx.get());
        sim.tick();
        
        REQUIRE(simulator.get_value(result) == 0b00001010_d);  // 10
    }
    
    SECTION("Arithmetic right shift with positive number") {
        ch_uint<8> a(0b01010000_d);  // 80 (positive)
    ...
    SECTION("Arithmetic right shift with negative number") {
        ...
        ch_uint<8> a(0b11111011_d);  // -5 in 2's complement
        ch_uint<8> result = arithmetic_right_shift<8>(a, 1);
        
        ch::Simulator sim(ctx.get());
        sim.tick();
        
        // Should preserve sign bit: 11111101 which is -3 in 2's complement
        REQUIRE(simulator.get_value(result) == 0b11111101_d);  // -3
    }
}
...
TEST_CASE("Arithmetic: edge cases", "[arithmetic][edge]") {
    ...
    SECTION("Maximum values addition") {
        ...
        REQUIRE(simulator.get_value(result) == 14);
    }
    ...
    SECTION("Zero operations") {
        ...
        REQUIRE(simulator.get_value(add_result) == 5);
        REQUIRE(simulator.get_value(sub_result) == 5);
    }
}