...
TEST_CASE("Logic: basic XOR gate function", "[logic][xor]") {
    ...
    SECTION("Simple XOR operation") {
        ...
        REQUIRE(sim.get_value(result) == 6_d);  // XOR 1100 (12) and 1010 (10) = 0110 (6)
    }
}
...
TEST_CASE("Logic: edge cases", "[logic][edge]") {
    ...
    SECTION("Single bit operations") {
        ...
        REQUIRE(sim.get_value(and_gate<1>(a, b)) == 0);
        REQUIRE(sim.get_value(or_gate<1>(a, b)) == 1);
        REQUIRE(sim.get_value(xor_gate<1>(a, b)) == 1);
        REQUIRE(sim.get_value(nand_gate<1>(a, b)) == 1);
    }
