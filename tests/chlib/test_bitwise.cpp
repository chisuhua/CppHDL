...
TEST_CASE("Bitwise: trailing zero detector", "[bitwise][trailing_zero]") {
    ...
    SECTION("Trailing zeros in 8-bit value") {
        ch_uint<8> input(10100000_b);
        ch_uint<4> result = trailing_zero_detector<8>(input);

        ch::Simulator sim(ctx.get());
        sim.tick();

        REQUIRE(sim.get_value(result) == 5); // 5 trailing zeros
    }
    ...
    SECTION("All zeros") {
        ...
        REQUIRE(sim.get_value(result) == 8); // 8 trailing zeros
    }
    ...
    SECTION("No trailing zeros") {
        ...
        REQUIRE(sim.get_value(result) == 0); // 0 trailing zeros
    }
    ...
    SECTION("Multiple trailing zeros") {
        ch_uint<8> input(11110000_b);
        ch_uint<4> result = trailing_zero_detector<8>(input);

        ch::Simulator sim(ctx.get());
        sim.tick();

        REQUIRE(sim.get_value(result) == 4); // 4 trailing zeros
    }
}
...
TEST_CASE("Bitwise: trailing one detector", "[bitwise][trailing_one]") {
    ...
    SECTION("Trailing ones in 8-bit value") {
        ch_uint<8> input(01011111_b);
        ch_uint<4> result = trailing_one_detector<8>(input);

        ch::Simulator sim(ctx.get());
        sim.tick();

        REQUIRE(sim.get_value(result) == 5); // 5 trailing ones
    }
    ...
    SECTION("All ones") {
        ...
        REQUIRE(sim.get_value(result) == 8); // 8 trailing ones
    }
    ...
    SECTION("No trailing ones") {
        ...
        REQUIRE(sim.get_value(result) == 0); // 0 trailing ones
    }
    ...
    SECTION("Single one at LSB") {
        ...
        REQUIRE(sim.get_value(result) == 1); // 1 trailing one
    }

    SECTION("Multiple trailing ones") {
        ch_uint<8> input(11110011_b);
        ch_uint<4> result = trailing_one_detector<8>(input);

        ch::Simulator sim(ctx.get());
        sim.tick();

        REQUIRE(sim.get_value(result) == 2); // 2 trailing ones
    }
}
...
TEST_CASE("Bitwise: population count", "[bitwise][popcount]") {
    ...
    SECTION("Count of 8-bit value with 3 ones") {
        ch_uint<8> input(10101000_b);
        ch_uint<4> result = population_count<8>(input);

        ch::Simulator sim(ctx.get());
        sim.tick();

        REQUIRE(sim.get_value(result) == 3); // 3 ones
    }
    ...
    SECTION("All zeros") {
        ...
        REQUIRE(sim.get_value(result) == 0); // 0 ones
    }
    ...
    SECTION("All ones") {
        ...
        REQUIRE(sim.get_value(result) == 8); // 8 ones
    }

    SECTION("Single one") {
        ch_uint<8> input(00010000_b);
        ch_uint<4> result = population_count<8>(input);

        ch::Simulator sim(ctx.get());
        sim.tick();

        REQUIRE(sim.get_value(result) == 1); // 1 one
    }

    SECTION("Alternating pattern") {
        ch_uint<8> input(10101010_b);
        ch_uint<4> result = population_count<8>(input);

        ch::Simulator sim(ctx.get());
        sim.tick();

        REQUIRE(sim.get_value(result) == 4); // 4 ones
    }
}
...
TEST_CASE("Bitwise: bit reversal", "[bitwise][bit_reversal]") {
    ...
    SECTION("Reversal of 8-bit value") {
        ch_uint<8> input(11001010_b); // 0b11001010
        ch_uint<8> result = bit_reversal<8>(input);

        ch::Simulator sim(ctx.get());
        sim.tick();

        REQUIRE(sim.get_value(result) == 0b01010011); // Reversed bits
    }
    ...
    SECTION("Reversal of all zeros") {
        ...
        REQUIRE(sim.get_value(result) == 0b00000000); // Still zeros
    }
    ...
    SECTION("Reversal of all ones") {
        ...
        REQUIRE(sim.get_value(result) == 0b11111111); // Still all ones
    }
    ...
    SECTION("Reversal of single bit") {
        ...
        REQUIRE(sim.get_value(result) == 0b00000001); // Bit moved to LSB
    }
}
...
TEST_CASE("Bitwise: bit swap", "[bitwise][bit_swap]") {
    ...
    SECTION("Swap same bit") {
        ...
        REQUIRE(sim.get_value(result) == 0b10110100);
    }
}
...
TEST_CASE("Bitwise: first set bit detector", "[bitwise][first_set_bit]") {
    ...
    SECTION("First set bit in 8-bit value") {
        ch_uint<8> input(00010100_b);
        ch_uint<4> result = first_set_bit_detector<8>(input);

        ch::Simulator sim(ctx.get());
        sim.tick();

        REQUIRE(sim.get_value(result) == 2); // First set bit at position 2
    }
    ...
    SECTION("All zeros") {
        ...
        REQUIRE(sim.get_value(result) == 8); // No set bit, returns N
    }
    ...
    SECTION("LSB set") {
        ...
        REQUIRE(sim.get_value(result) == 0); // LSB at position 0
    }
    ...
    SECTION("MSB set") {
        ...
        REQUIRE(sim.get_value(result) == 7); // MSB at position 7
    }
}
...
TEST_CASE("Bitwise: bit field insert", "[bitwise][bit_field_insert]") {
    ...
    SECTION("Insert with no change") {
        ch_uint<8> input(11010110_b);
        ch_uint<8> value_to_insert(
            101_b); // 3 bits: 101 (same as original at pos 2-4)
        ...
        REQUIRE(sim.get_value(result) == 0b11010110);
    }
}