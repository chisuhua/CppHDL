...
TEST_CASE("Arithmetic Advance: carry lookahead adder", "[arithmetic_advance][cla]") {
...
TEST_CASE("Arithmetic Advance: carry save adder", "[arithmetic_advance][csa]") {
    ...
    SECTION("Three operand addition") {
        ...
        REQUIRE(simulator.get_value(result.sum) == (5 ^ 3 ^ 2));  // 5^3^2 = 4
        REQUIRE(simulator.get_value(result.carry) == ((5 & 3) | (3 & 2) | (5 & 2)) << 1);  // Carry bits shifted
    }
}

TEST_CASE("Arithmetic Advance: wallace tree multiplier", "[arithmetic_advance][wallace]") {
    ...
    SECTION("Simple multiplication") {
        ...
        REQUIRE(simulator.get_value(result) == 15);  // 5 * 3 = 15
    }
    ...
    SECTION("Multiplication by zero") {
        ...
        REQUIRE(simulator.get_value(result) == 0);  // 7 * 0 = 0
    }
    ...
    SECTION("Multiplication by one") {
        ...
        REQUIRE(simulator.get_value(result) == 9);  // 9 * 1 = 9
    }
}

TEST_CASE("Arithmetic Advance: booth multiplier", "[arithmetic_advance][booth]") {
...
TEST_CASE("Arithmetic Advance: non-restoring divider", "[arithmetic_advance][divider]") {
    ...
    SECTION("Simple division") {
        ...
        REQUIRE(simulator.get_value(result.quotient) == 4);   // 12 / 3 = 4
        REQUIRE(simulator.get_value(result.remainder) == 0);  // 12 % 3 = 0
    }
    ...
    SECTION("Division with remainder") {
        ...
        REQUIRE(simulator.get_value(result.quotient) == 4);   // 13 / 3 = 4
        REQUIRE(simulator.get_value(result.remainder) == 1);  // 13 % 3 = 1
    }
    ...
    SECTION("Division by zero") {
        ...
        REQUIRE(simulator.get_value(result.quotient) == 15);  // Max value for 4-bit
        REQUIRE(simulator.get_value(result.remainder) == 0);  // Remainder is 0
    }
}

TEST_CASE("Arithmetic Advance: square root calculator", "[arithmetic_advance][sqrt]") {
    ...
    SECTION("Perfect square") {
        ...
        REQUIRE(simulator.get_value(result) == 4);  // sqrt(16) = 4
    }
    ...
    SECTION("Non-perfect square") {
        ...
        REQUIRE(simulator.get_value(result) == 3);  // floor(sqrt(10)) = 3
    }
    ...
    SECTION("Square root of 0") {
        ...
        REQUIRE(simulator.get_value(result) == 0);  // sqrt(0) = 0
    }
    ...
    SECTION("Square root of 1") {
        ...
        REQUIRE(simulator.get_value(result) == 1);  // sqrt(1) = 1
    }
}

TEST_CASE("Arithmetic Advance: fixed point arithmetic", "[arithmetic_advance][fixed_point]") {
    auto ctx = std::make_unique<ch::core::context>("test_fixed_point");
    ch::core::ctx_swap ctx_swapper(ctx.get());

    SECTION("Fixed point addition (Q8.4 format)") {
        ch_uint<8> a(0b00010000_d);  // 1.0 in Q4.4 format
        ch_uint<8> b(0b00001000_d);  // 0.5 in Q4.4 format
        ...
        REQUIRE(simulator.get_value(result.result) == 24);
    }
    ...
    SECTION("Fixed point multiplication (Q8.4 format)") {
        ...
        REQUIRE(simulator.get_value(result.result) == 8);
    }
    ...
    SECTION("Fixed point division (Q8.4 format)") {
        ...
        REQUIRE(simulator.get_value(result.result) == 32);
    }
}