...
TEST_CASE("Sequential: register function", "[sequential][register]") {
    ...
    SECTION("Basic register functionality") {
        ...
        REQUIRE(sim.get_value(q) == 5); // Value after clock
    }
...
TEST_CASE("Sequential: binary counter function",
    ...
    SECTION("4-bit binary counter") {
        ...
        ch_uint<4> count =
            binary_counter<4>(clk, rst, en, "test_binary_counter");
...
TEST_CASE("Sequential: BCD counter function", "[sequential][bcd_counter]") {
    ...
    SECTION("4-bit BCD counter") {
        ...
        BCDCounterResult<4> result =
            bcd_counter<4>(clk, rst, en, "test_bcd_counter");
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            if (i == 8) { // When counter was 9 and will reset to 0
                REQUIRE(sim.get_value(result.carry) == false);
            }
        }
        ...
        REQUIRE(sim.get_value(result.carry) == true); // Carry generated
    }
}
...
TEST_CASE("Sequential: counter function", "[sequential][counter]") {
    ...
    SECTION("Down counter") {
        ...
        ch_uint<4> count =
            counter<4>(clk, rst, en, up_down, "test_counter_down");
...
TEST_CASE("Sequential: ring counter function", "[sequential][ring_counter]") {
    ...
    SECTION("4-bit ring counter") {
        ...
        REQUIRE(sim.get_value(out) == 1); // 0001
...
TEST_CASE("Sequential: shift register function",
    ...
    SECTION("Left shift operation") {
        ...
        ShiftRegisterResult<4> sreg = shift_register<4>(
            clk, rst, en, shift_dir, parallel_in, load, "test_shift_reg");
        ...
        REQUIRE(sim.get_value(sreg.serial_out) == true); // MSB was 1
    }
    ...
    SECTION("Right shift operation") {
        ...
        ch_bool load = false;
        ch_uint<4> parallel_in = 1100_b;

        ShiftRegisterResult<4> sreg = shift_register<4>(
            clk, rst, en, shift_dir, parallel_in, load, "test_shift_reg_right");
        ...
        REQUIRE(sim.get_value(sreg.out) == 1100_b);
        ...
        REQUIRE(sim.get_value(sreg.out) == 0b0110);
        REQUIRE(sim.get_value(sreg.serial_out) == false); // LSB was 0
    }
}
...
TEST_CASE("Sequential: edge detector function", "[sequential][edge_detector]") {
    ...
    SECTION("Rising edge detection") {
        ...
        EdgeDetectorResult detector =
            edge_detector(clk, rst, signal, "test_edge_detector");

        ch::Simulator sim(ctx.get());
        sim.tick();

        REQUIRE(sim.get_value(detector.pos_edge) == false);
        REQUIRE(sim.get_value(detector.neg_edge) == false);
        REQUIRE(sim.get_value(detector.any_edge) == false);
    ...
    SECTION("Falling edge detection") {
        ...
        EdgeDetectorResult detector =
            edge_detector(clk, rst, signal, "test_edge_detector_fall");
        ...
        REQUIRE(sim.get_value(detector.pos_edge) == false);
        REQUIRE(sim.get_value(detector.neg_edge) == true);
        REQUIRE(sim.get_value(detector.any_edge) == true);
    }
}
...
TEST_CASE("Sequential: configurable counter function",
    ...
    SECTION("Up counter mode") {
        ...
        ConfigurableCounterResult<4> counter = configurable_counter<4>(
            clk, rst, en, mode, max_val, "test_config_counter");
    ...
    SECTION("Down counter mode") {
        ...
        ConfigurableCounterResult<4> counter = configurable_counter<4>(
            clk, rst, en, mode, max_val, "test_config_counter_down");
...
TEST_CASE("Sequential: edge cases", "[sequential][edge]") {
    ...
    SECTION("Counter overflow behavior") {
        ...
        ch_uint<3> count =
            counter<3>(clk, rst, en, up_down, "test_counter_overflow");
