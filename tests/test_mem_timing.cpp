...
public:
    __io(ch_in<ch_uint<8>> addr_in; ch_in<ch_uint<32>> data_in;
         ch_in<ch_bool> we_in; // Write enable
         ch_in<ch_bool> en_in; // Enable for reads
         ch_out<ch_uint<32>> data_out; ch_out<ch_bool> done_out;)
    ...
    void describe() override {
        ...
        auto read_port = mem.sread(ch_uint<8>(io().addr_in.impl()),
                                   ch_bool(io().en_in.impl()));
...
public:
    __io(ch_in<ch_uint<8>> read_addr; ch_in<ch_uint<8>> write_addr;
         ch_in<ch_uint<32>> write_data; ch_in<ch_bool> write_enable;
         ch_out<ch_uint<32>> read_data; ch_out<ch_bool> read_valid;)
    ...
    void describe() override {
        ...
        // Read port
        auto read_port =
            mem.sread(ch_uint<8>(io().read_addr.impl()), ch_bool(true));
...
public:
    __io(ch_in<ch_uint<8>> addr_in; ch_out<ch_uint<32>> data_out;)
    ...
    void describe() override {
        ...
        std::vector<uint32_t> init_data = {0xDEADBEEF, 0x12345678, 0xABCDEF00,
                                           0xFEDCBA98};
...
public:
    __io(ch_in<ch_uint<8>> addr_in; ch_in<ch_uint<32>> data_in;
         ch_in<ch_bool> write_enable; ch_out<ch_uint<32>> data_out;)
    ...
    void describe() override {
        ...
        auto read_port =
            mem.sread(ch_uint<8>(io().addr_in.impl()), ch_bool(true));
...
TEST_CASE("Memory Timing - Single Port Memory Write/Read",
    ...
    simulator.set_input_value(device.instance().io().data_in,
                              static_cast<uint64_t>(0x12345678));
    simulator.set_input_value(device.instance().io().we_in,
                              static_cast<uint64_t>(1)); // Write enable
    simulator.set_input_value(device.instance().io().en_in,
                              static_cast<uint64_t>(0)); // Disable read
    ...
    simulator.set_input_value(device.instance().io().we_in,
                              static_cast<uint64_t>(0)); // Read mode
    simulator.set_input_value(device.instance().io().en_in,
                              static_cast<uint64_t>(1)); // Enable read
    simulator.tick();
    ...
    simulator.set_input_value(device.instance().io().data_in,
                              static_cast<uint64_t>(0xABCDEF00));
    simulator.set_input_value(device.instance().io().we_in,
                              static_cast<uint64_t>(1)); // Write mode
    simulator.set_input_value(device.instance().io().en_in,
                              static_cast<uint64_t>(0)); // Disable read
    simulator.tick();
...
TEST_CASE("Memory Timing - Initialized Memory", "[mem][timing][initialized]") {
    ...
    write_simulator.set_input_value(write_device.instance().io().addr_in,
                                    static_cast<uint64_t>(0));
    write_simulator.set_input_value(write_device.instance().io().data_in,
                                    static_cast<uint64_t>(0x99999999));
    write_simulator.set_input_value(write_device.instance().io().we_in,
                                    static_cast<uint64_t>(1));
    write_simulator.set_input_value(write_device.instance().io().en_in,
                                    static_cast<uint64_t>(1));
    ...
    read_value =
        write_simulator.get_port_value(write_device.instance().io().data_out);
...
TEST_CASE("Memory Timing - Write-Read Dependency",
          "[mem][timing][dependency]") {
    ...
    simulator.set_input_value(device.instance().io().addr_in,
                              static_cast<uint64_t>(10));
    simulator.set_input_value(device.instance().io().data_in,
                              static_cast<uint64_t>(0x55555555));
    simulator.set_input_value(device.instance().io().write_enable,
                              static_cast<uint64_t>(1));
    simulator.tick();

    // Try to read from the same address in the same cycle (depends on
    // implementation) In most memory implementations, this would return the old
    // value
    auto read_value = simulator.get_port_value(device.instance().io().data_out);
...
TEST_CASE("Memory Timing - Sequential Writes and Reads",
          "[mem][timing][sequential]") {
    ch_device<SinglePortMem> device;
    Simulator simulator(device.context());
    ...
    for (int addr = 0; addr < 10; addr++) {
        ...
        simulator.set_input_value(device.instance().io().data_in,
                                  static_cast<uint64_t>(0x2000 + addr));
        simulator.set_input_value(device.instance().io().we_in,
                                  static_cast<uint64_t>(1));
        simulator.set_input_value(device.instance().io().en_in,
                                  static_cast<uint64_t>(0));
    ...
    for (int addr = 0; addr < 10; addr++) {
        ...
        auto read_value =
            simulator.get_port_value(device.instance().io().data_out);
        REQUIRE(static_cast<uint64_t>(read_value) ==
                static_cast<uint64_t>(0x2000 + addr));
    }
    ...
    for (int addr = 0; addr < 10; addr++) {
        simulator.set_input_value(device.instance().io().addr_in,
                                  static_cast<uint64_t>(addr));
        simulator.set_input_value(device.instance().io().data_in,
                                  static_cast<uint64_t>(0x3000 + addr));
        simulator.set_input_value(device.instance().io().we_in,
                                  static_cast<uint64_t>(1));
        simulator.set_input_value(device.instance().io().en_in,
                                  static_cast<uint64_t>(0));
