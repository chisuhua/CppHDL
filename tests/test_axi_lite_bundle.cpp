...
TEST_CASE("AXILiteBundle - ProtocolValidation", "[axi][lite][protocol]") {
    ...
    STATIC_REQUIRE(is_axi_lite_write_v<axi_lite_write_interface<32, 32>> == true);
    STATIC_REQUIRE(is_axi_lite_read_v<axi_lite_read_interface<32, 32>> == true);
    
    // 非AXI类型检查
    STATIC_REQUIRE(is_axi_lite_v<stream_bundle<ch_uint<32>>> == false);
    STATIC_REQUIRE(is_axi_lite_write_v<stream_bundle<ch_uint<32>>> == false);
    STATIC_REQUIRE(is_axi_lite_read_v<stream_bundle<ch_uint<32>>> == false);
}
...
TEST_CASE("AXILiteBundle - FieldNameChecking", "[axi][lite][fields]") {
    ...
    // 测试字段存在性检查
    STATIC_REQUIRE(has_field_named_v<axi_lite_bundle<32, 32>, structural_string{"aw"}> == true);
    STATIC_REQUIRE(has_field_named_v<axi_lite_bundle<32, 32>, structural_string{"w"}> == true);
    STATIC_REQUIRE(has_field_named_v<axi_lite_bundle<32, 32>, structural_string{"b"}> == true);
    STATIC_REQUIRE(has_field_named_v<axi_lite_bundle<32, 32>, structural_string{"ar"}> == true);
    STATIC_REQUIRE(has_field_named_v<axi_lite_bundle<32, 32>, structural_string{"r"}> == true);
    STATIC_REQUIRE(has_field_named_v<axi_lite_bundle<32, 32>, structural_string{"invalid"}> == false);
}
...
TEST_CASE("AXILiteBundle - ProtocolValidationFunctions", "[axi][lite][validation]") {
    auto ctx = std::make_unique<ch::core::context>("test_ctx");
    ch::core::ctx_swap ctx_guard(ctx.get());
