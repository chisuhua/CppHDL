...
public:
    __io(ch_out<ch_uint<8>> test_out; ch_in<ch_uint<8>> test_in;
         ch_out<ch_bool> bool_out; ch_in<ch_bool> bool_in;)

        TestComponent(ch::Component *parent = nullptr,
                      const std::string &name = "test_component")
...
TEST_CASE("Simulator: basic functionality", "[simulator][basic]") {
    // Don't skip simulator test - just test basic functionality to avoid
    // segfaults
...
TEST_CASE("Width traits", "[traits][width]") {
    ...
    SECTION("ch_bool width trait") { REQUIRE(ch_width_v<ch_bool> == 1); }
