...
TEST_CASE("BundleAdvanced - ProtocolValidation", "[bundle][protocol]") {
    ...
    // 验证HandShake协议
    STATIC_REQUIRE(is_handshake_protocol_v<stream_bundle<ch_uint<32>>> == true);
    STATIC_REQUIRE(is_handshake_protocol_v<ch_uint<32>> == false);
    
    // 验证字段检查
    STATIC_REQUIRE(has_field_named_v<stream_bundle<ch_uint<32>>, structural_string{"payload"}> == true);
    STATIC_REQUIRE(has_field_named_v<stream_bundle<ch_uint<32>>, structural_string{"valid"}> == true);
    STATIC_REQUIRE(has_field_named_v<stream_bundle<ch_uint<32>>, structural_string{"ready"}> == true);
    STATIC_REQUIRE(has_field_named_v<stream_bundle<ch_uint<32>>, structural_string{"nonexistent"}> == false);
}
/*
TEST_CASE("BundleAdvanced - BundleOperations", "[bundle][operations]") {
    ...
    // 测试Bundle连接 (简化版)
    stream_bundle<ch_uint<16>> stream1;
    stream_bundle<ch_uint<16>> stream2;
...
TEST_CASE("BundleAdvanced - TypeTraits", "[bundle][traits]") {
    ...
    stream_bundle<ch_uint<8>> stream;
...
TEST_CASE("BundleAdvanced - ProtocolCheckFunction", "[bundle][protocol][function]") {
    auto ctx = std::make_unique<ch::core::context>("test_ctx");
    ch::core::ctx_swap ctx_guard(ctx.get());
