...
TEST_CASE("IOExtendedOps - BitSelectOperation", "[io][extended_ops][bit_select]") {
    auto ctx = context("test_ctx");
    
    SECTION("Compile-time bit selection") {
        ch_out<ch_uint<8>> port("test_port");
        auto bit0 = bit_select<ch_uint<8>, 0>(port);
        auto bit7 = bit_select<ch_uint<8>, 7>(port);
...
TEST_CASE("IOExtendedOps - ReductionOperations", "[io][extended_ops][reduction]") {
    auto ctx = context("test_ctx");
...
TEST_CASE("IOExtendedOps - ExtensionOperations", "[io][extended_ops][extension]") {
    auto ctx = context("test_ctx");
