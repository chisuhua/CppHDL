...
TEST_CASE("precise_width: concat operation with exact widths",
          "[precise_width][concat]") {
    // 创建测试上下文
...
TEST_CASE("precise_width: nested concat operations",
          "[precise_width][concat][nested]") {
    // 创建测试上下文
    context ctx("test_ctx");
    ctx_swap swap(&ctx);
...
TEST_CASE("precise_width: bit operations", "[precise_width][bits]") {
    ...
    // 测试位提取操作
    auto slice_7_4 = bits<7, 4>(data); // 应该是4位
...
TEST_CASE("precise_width: arithmetic operations",
          "[precise_width][arithmetic]") {
    // 创建测试上下文
    context ctx("test_ctx");
    ctx_swap swap(&ctx);
